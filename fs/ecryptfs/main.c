...
static int parse_enc_folder_filter_parms(struct ecryptfs_mount_crypt_stat *mcs, char *str)
{
        ...
        (void)memset_s(mcs->enc_filter_folder_name, sizeof(mcs->enc_filter_folder_name), 0, sizeof(mcs->enc_filter_folder_name));
		while ((token = strsep(&str, "|")) != NULL) {
                ...
                if(strlen(token) >= ENC_FOLDER_FILTER_MAX_LEN){
                    ...
                    ecryptfs_printk(KERN_WARNING,"ENCRYPTSD_FILTER parse_enc_file_filter_parms space happened\n");
                        ...
                        if (strncpy_s(mcs->enc_filter_folder_name[count++], sizeof(mcs->enc_filter_folder_name[count++]), token, strlen(token))) {
                                return -1;
                        }
                }
        }
