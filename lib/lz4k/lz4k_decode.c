...
{
	if (!nr_bytes_max)
		return STATUS_FAILED; /* should be the last one in block */
	...
	const uint_fast32_t off_log2)
{
	...
	while (likely(in_at <= in_end_minus_x)) {
		...
		const uint8_t *const out_from = out_at - offset;
		if (unlikely(out_from < (out0 < out ? out0 : out)))
			return STATUS_FAILED;
		uint8_t *const out_copy_end = out_at + r_bytes_max;
	...
	return in_at == in_end ? (int)(out_at - out) : STATUS_FAILED;
} /* decode */
	...
	unsigned out_max)
{
	...
	if (unlikely((const uint8_t*)in >= in_end || (uint8_t*)out >= out_end))
		return STATUS_FAILED;
	/* -- */
	const uint8_t in_log2 = (uint8_t)(BLOCK_4KB_LOG2 + *(const uint8_t*)in);
	++in;
	--in_max;
