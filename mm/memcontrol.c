...
#ifdef CONFIG_HYPERHOLD_FILE_LRU
	if (mem_cgroup_disabled() || is_file_lru(page_lru(page))) {
#else
	if (mem_cgroup_disabled()) {
#endif
		lruvec = &pgdat->lruvec;
				...
				struct cftype *cft, u64 val)
{
	...
	if (val > ratio)
		return -EINVAL;
	atomic64_set(&memcg->ub_ufs2zram_ratio, val);
				...
				struct cftype *cft, u64 val)
{
	...
	size = atomic64_read(&memcg->ub_ufs2zram_ratio) * size / ratio;
...
static void memcg_app_score_update(struct mem_cgroup *target)
{
	...
	list_for_each(pos, &score_head) {
		struct mem_cgroup *memcg = list_entry(pos,
				struct mem_cgroup, score_node);
		if (atomic64_read(&memcg->app_score) <
				atomic64_read(&target->app_score))
			break;
	}
				...
				struct cftype *cft, u64 val)
{
	...
	if (atomic64_read(&memcg->app_score) != val) {
		atomic64_set(&memcg->app_score, val);
...
EXPORT_SYMBOL(zswapd_pressure_report);
	...
	while ((memcg = get_next_memcg(memcg))) {
		for (i = 0; i < level_num; ++i)
			if ((atomic64_read(&memcg->app_score) >=
			     ...
			     atomic64_read(&memcg->app_score) <=
					zswap_param[i].max_score)
				break;

		atomic_set(&memcg->ub_mem2zram_ratio,
			zswap_param[i].ub_mem2zram_ratio);
		atomic_set(&memcg->ub_zram2ufs_ratio,
			zswap_param[i].ub_zram2ufs_ratio);
		atomic_set(&memcg->refault_threshold,
				...
				char *buf, size_t nbytes, loff_t off)
{
	...
	atomic_set(&memcg->ub_mem2zram_ratio, ub_mem2zram_ratio);
	atomic_set(&memcg->ub_zram2ufs_ratio, ub_zram2ufs_ratio);
	atomic_set(&memcg->refault_threshold, refault_threshold);
...
static int memcg_active_app_info_list_show(struct seq_file *m, void *v)
{
	...
	while ((memcg = get_next_memcg(memcg))) {
		u64 score = atomic64_read(&memcg->app_score);
				...
				struct cftype *cft)
{
	...
	return atomic64_read(&memcg->ub_ufs2zram_ratio);
}
...
static int zswapd_single_memcg_param_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "memcg ub_mem2zram_ratio: %u\n",
			atomic_read(&memcg->ub_mem2zram_ratio));
	seq_printf(m, "memcg ub_zram2ufs_ratio: %u\n",
			atomic_read(&memcg->ub_zram2ufs_ratio));
	seq_printf(m, "memcg refault_threshold: %u\n",
			atomic_read(&memcg->refault_threshold));
...
static int memcg_score_list_show(struct seq_file *m, void *v)
{
	...
	while ((memcg = get_next_memcg(memcg)))
		seq_printf(m, "%lu %s\n",
			atomic64_read(&memcg->app_score), memcg->name);
				...
				struct cftype *cft)
{
	...
	return atomic64_read(&memcg->app_score);
}
...
#ifdef CONFIG_HYPERHOLD
	atomic64_set(&memcg->app_score, 300);
	atomic64_set(&memcg->ub_ufs2zram_ratio, 100);
#ifdef CONFIG_HYPERHOLD_ZSWAPD
	atomic_set(&memcg->ub_zram2ufs_ratio, 10);
	atomic_set(&memcg->ub_mem2zram_ratio, 60);
	atomic_set(&memcg->refault_threshold, 50);
#endif
#endif
