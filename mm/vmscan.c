...
static void shrink_anon_memcg(struct pglist_data *pgdat,
...
static void shrink_file(struct pglist_data *pgdat,
		struct scan_control *sc, unsigned long *nr);
static bool shrink_node_hyperhold(pg_data_t *pgdat,
...
unsigned long lruvec_lru_size(struct lruvec *lruvec, enum lru_list lru, int zone_idx)
{
	...
	int zid;
#ifdef CONFIG_HYPERHOLD_FILE_LRU
	if (!mem_cgroup_disabled() && !is_file_lru(lru))
#else
		     ...
		     struct scan_control *sc, enum lru_list lru)
{
	...
	bool stalled = false;
#ifdef CONFIG_HYPERHOLD_FILE_LRU
	struct lruvec *lruvec_local = lruvec;
	struct zone_reclaim_stat *node_reclaim_stat;

	node_reclaim_stat = &pgdat->lruvec.reclaim_stat;

	if (file)
		lruvec_local = &pgdat->lruvec;
#endif
...
#ifdef CONFIG_HYPERHOLD_FILE_LRU
	nr_taken = isolate_lru_pages(nr_to_scan, lruvec_local, &page_list,
...
#ifdef CONFIG_HYPERHOLD_FILE_LRU
	putback_inactive_pages(lruvec_local, &page_list);
#else
			       ...
			       enum lru_list lru)
{
...
#ifdef CONFIG_HYPERHOLD_FILE_LRU
	nr_taken = isolate_lru_pages(nr_to_scan, lruvec_local, &l_hold,
				     &nr_scanned, sc, isolate_mode, lru);
#else
	...
	nr_activate = move_active_pages_to_lru(lruvec_local, &l_active,
			&l_hold, lru);
	nr_deactivate = move_active_pages_to_lru(lruvec_local, &l_inactive,
...
#ifdef CONFIG_HYPERHOLD
unsigned long move_pages_to_page_list(struct lruvec *lruvec,
...
#ifdef CONFIG_HYPERHOLD_FILE_LRU
unsigned long node_lruvec_lru_size(struct pglist_data *pgdat,
		enum lru_list lru, int zone_idx)
{
	...
	for (zid = zone_idx + 1; zid < MAX_NR_ZONES; zid++) {
		...
		size = zone_page_state(&pgdat->node_zones[zid],
				NR_ZONE_LRU_BASE + lru);
		lru_size -= min(size, lru_size);
	}
...
static bool node_inactive_list_is_low(struct pglist_data *pgdat, bool file,
		struct scan_control *sc, bool actual_reclaim)
{
	...
	inactive = node_lruvec_lru_size(pgdat, inactive_lru, sc->reclaim_idx);
	active = node_lruvec_lru_size(pgdat, active_lru, sc->reclaim_idx);

	refaults = node_page_state(pgdat, WORKINGSET_ACTIVATE);
	if (file && lruvec->refaults != refaults) {
		inactive_ratio = 0;
	} else {
		gb = (inactive + active) >> (30 - PAGE_SHIFT); /* page to GB */
		if (gb && !file)
			inactive_ratio = int_sqrt(10 * gb); /* 10 is a empirical value */
		else
			inactive_ratio = 1;
	}
		...
		unsigned long *lru_pages)
{
	...
	if (unlikely(pgdatfile + pgdatfree <= total_high_wmark)) {
		 ...
		 * Otherwise, the small LRU gets thrashed.
		 */
		if (!node_inactive_list_is_low(pgdat, false, sc, false) &&
			(node_lruvec_lru_size(pgdat, LRU_INACTIVE_ANON,
	 ...
	 */
	if (enough_inactive_file == 0 &&
		!node_inactive_list_is_low(pgdat, true, sc, false) &&
		(node_lruvec_lru_size(pgdat, LRU_INACTIVE_FILE,
			sc->reclaim_idx) >> sc->priority)) {
		scan_balance = SCAN_FILE;
		goto out;
	}
	...
	anon  = node_lruvec_lru_size(pgdat, LRU_ACTIVE_ANON, MAX_NR_ZONES) +
		node_lruvec_lru_size(pgdat, LRU_INACTIVE_ANON, MAX_NR_ZONES);
	file  = node_lruvec_lru_size(pgdat, LRU_ACTIVE_FILE, MAX_NR_ZONES) +
		node_lruvec_lru_size(pgdat, LRU_INACTIVE_FILE, MAX_NR_ZONES);
...
out:
	*lru_pages = 0;
	for_each_evictable_lru(lru) {
		...
		size = node_lruvec_lru_size(pgdat, lru, sc->reclaim_idx);
		scan = size >> sc->priority;
		/*
		 * If the cgroup's already been deleted, make sure to
		 * scrape out the remaining cache.
		...
		struct scan_control *sc, unsigned long *nr)
{
	...
	unsigned long nr_node_active = node_lruvec_lru_size(pgdat,
			LRU_ACTIVE_ANON, MAX_NR_ZONES);
	unsigned long nr_node_inactive = node_lruvec_lru_size(pgdat,
			LRU_INACTIVE_ANON, MAX_NR_ZONES);
...
static bool shrink_node_hyperhold(pg_data_t *pgdat, struct scan_control *sc)
{
	struct reclaim_state *reclaim_state = current->reclaim_state;
	unsigned long nr_reclaimed, nr_scanned;
	bool reclaimable = false;
...
EXPORT_SYMBOL(zswapd_status_show);

pid_t get_zswapd_pid(void)
{
	return zswapd_pid;
}
EXPORT_SYMBOL(get_zswapd_pid);
...
static bool get_memcg_anon_refault_status(struct mem_cgroup *memcg)
{
	...
	cur_anon_pagefault = hyperhold_read_mcg_stats(memcg, MCG_ANON_FAULT_CNT);

	if (cur_anon_pagefault == memcg->reclaimed_pagefault)
		return false;
	...
	ratio = (cur_anon_pagefault - memcg->reclaimed_pagefault) *
		percent_constant / (anon_total + 1);

	if (ratio > atomic_read(&memcg->refault_threshold))
		return true;
...
int get_zram_current_watermark(void)
{
	...
	diff_buffers *= get_compress_ratio_value(); /* after_comp to before_comp */
...
{
	u64 nr_zram_used;
	u64 nr_wm = get_zram_critical_threshold_value() * (SZ_1M / PAGE_SIZE);
	...
	struct lruvec *lruvec, struct scan_control *sc, enum lru_list lru)
{
	...
	nr_deactivate = move_active_pages_to_lru(lruvec, &l_inactive,
			&l_hold, lru - LRU_ACTIVE);
	__mod_node_page_state(pgdat, NR_ISOLATED_ANON, -nr_taken); /*lint !e501*/
	spin_unlock_irq(&pgdat->lru_lock);
	mem_cgroup_uncharge_list(&l_hold);
	free_hot_cold_page_list(&l_hold, true);
...
static bool zswapd_shrink_anon(pg_data_t *pgdat, struct scan_control *sc)
{
	...
	while ((memcg = get_next_memcg(memcg))) {
		...
		nr_eswap = hyperhold_read_mcg_stats(memcg, MCG_DISK_STORED_PG_SZ);

		zram_ratio = (nr_zram + nr_eswap) * percent_constant /
			(nr_inactive + nr_active + nr_zram + nr_eswap + 1);

		if (zram_ratio >= atomic_read(&memcg->ub_mem2zram_ratio)) { /*lint !e574*/
			count_vm_event(ZSWAPD_MEMCG_RATIO_SKIP);
			continue;
		}

		nr[LRU_ACTIVE_ANON] = nr_active >> sc->priority;
		nr[LRU_INACTIVE_ANON] = nr_inactive >> sc->priority;
		nr[LRU_ACTIVE_FILE] = 0;
		nr[LRU_INACTIVE_FILE] = 0;
#ifdef CONFIG_HYPERHOLD_FILE_LRU
...
static int zswapd(void *p)
{
	...
	while (!kthread_should_stop()) {
		...
		wait_event_freezable(pgdat->zswapd_wait,
