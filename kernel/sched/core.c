...
#endif /* CONFIG_SCHED_HISI_TASK_MIN_UTIL */
				  ...
				  const struct cpumask *new_mask, bool check)
{
	...
	struct rq *rq;
	int ret = 0;
#if defined(CONFIG_HISI_CPU_ISOLATION_STRICT) || defined(CONFIG_HISI_EAS_SCHED)
			...
			struct task_struct *prev, struct task_struct *next)
{
	...
	if (!next->hisi_vip_prio && next->sched_class != &rt_sched_class)
