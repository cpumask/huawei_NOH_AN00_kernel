...
#ifdef CONFIG_HISI_EAS_SCHED
#ifdef CONFIG_SCHED_HISI_DEPRECATED
...
#if defined(CONFIG_SCHED_WALT) && defined(CONFIG_SCHED_HISI_USE_WALT)
...
static void enqueue_hisi_vip_thread(struct rq *rq, struct task_struct *p)
{
	...
	if (p->hisi_vip_prio > rq->highest_hisi_vip_prio)
		rq->highest_hisi_vip_prio = p->hisi_vip_prio;
}
...
static void dequeue_hisi_vip_thread(struct rq *rq, struct task_struct *p)
{
	...
	list_for_each_entry(task, &rq->hisi_vip_thread_list, hisi_vip_entry) {
		highest_prio = max(highest_prio, task->hisi_vip_prio);
...
pick_next_hisi_vip_thread(struct rq *rq)
{
	...
	list_for_each_entry_safe(task, n, &rq->hisi_vip_thread_list, hisi_vip_entry) {
		...
		if (!leftmost_task ||
		    task->hisi_vip_prio > leftmost_task->hisi_vip_prio ||
		    (task->hisi_vip_prio == leftmost_task->hisi_vip_prio &&
			...
			struct cpumask *search_cpus)
{
	int i;
	int target_cpu = -1;
	unsigned int lowest_prio = p->hisi_vip_prio;
...
static inline bool check_vip_contend(int cpu, struct task_struct *p)
{
	...
	if (p->hisi_vip_prio < rq->highest_hisi_vip_prio)
		return true;
	else if (p->hisi_vip_prio > rq->highest_hisi_vip_prio)
		return false;
				   ...
				   bool boosted, bool prefer_idle)
{
	...
	*backup_cpu = -1;
	...
	if (boosted) {
		target_capacity = 0;
#ifndef CONFIG_SCHED_HISI_DEPRECATED
		prefer_idle = true;
#endif
	}
#else
...
#ifndef CONFIG_SCHED_HISI_USE_WALT
			/*
			 * Enforce EAS mode
			 *
...
static inline bool nohz_kick_needed(struct rq *rq, bool only_update)
{
	...
	bool kick = false;
#if defined(CONFIG_HISI_CPU_ISOLATION) && !defined(CONFIG_HISI_EAS_SCHED)
	cpumask_t cpumask;
#endif
