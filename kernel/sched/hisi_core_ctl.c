...
static unsigned int g_rq_avg_update_interval_ms = UPDATE_INIT_20MS;
static s64 g_rq_avg_update_timestamp_ms;
					...
					const char *buf, size_t count)
{
	...
	spin_lock_irqsave(&state_lock, flags);
	g_rq_avg_update_interval_ms = clamp(val, UPDATE_INTERVAL_MIN,
					    UPDATE_INTERVAL_MAX);
	spin_unlock_irqrestore(&state_lock, flags);
			       ...
			       struct sched_nr_stat *stat)
{
	...
	stat->iowait_avg = (unsigned int)div64_u64(tmp_iowait * NR_AVG_AMP,
						   diff);
...
static int update_running_avg(void)
{
	...
	if (now < g_rq_avg_update_timestamp_ms + g_rq_avg_update_interval_ms) {
		ret = -EPERM;
		goto check_misfit;
	}

	g_rq_avg_update_timestamp_ms = now;
