...
static int scp_protocol_get_vendor_id(int *id)
{
	...
	/* reg: 0x82 & 0x83 */
	if (scp_protocol_reg_multi_read(SCP_PROTOCOL_VENDOR_ID_HBYTE,
		value, BYTE_TWO))
		return -1;
...
static int scp_protocol_get_module_id(int *id)
{
	...
	/* reg: 0x84 & 0x85 */
	if (scp_protocol_reg_multi_read(SCP_PROTOCOL_MODULE_ID_HBYTE,
		value, BYTE_TWO))
		return -1;
...
static int scp_protocol_get_serial_no_id(int *id)
{
	...
	/* reg: 0x86 & 0x87 */
	if (scp_protocol_reg_multi_read(SCP_PROTOCOL_SERIAL_NO_HBYTE,
		value, BYTE_TWO))
		return -1;
...
static int scp_protocol_get_output_current_d(int *cur)
{
	...
	/* reg: 0xaa & 0xab */
	if (scp_protocol_reg_multi_read(SCP_PROTOCOL_READ_IOUT_HBYTE,
		value, BYTE_TWO))
		return -1;
...
static int scp_protocol_get_output_current_set_d(int *cur)
{
	...
	/* reg: 0xba & 0xbb */
	if (scp_protocol_reg_multi_read(SCP_PROTOCOL_ISET_HBYTE, value, BYTE_TWO))
		return -1;
...
static int scp_protocol_get_random_num(int retrys)
{
	...
	/* reg: 0xa8 */
	for (i = 0; i < retrys; i++) {
		if (scp_protocol_reg_multi_read(SCP_PROTOCOL_ENCRYPT_RANDOM_RD_BASE,
			value, BYTE_EIGHT) == 0)
			break;
	}
...
static int scp_protocol_get_encrypted_value(int retrys)
{
	...
	/* reg: 0xb0 */
	for (i = 0; i < retrys; i++) {
		if (scp_protocol_reg_multi_read(SCP_PROTOCOL_ENCRYPT_HASH_RD_BASE,
...
static int scp_protocol_set_init_data(struct adapter_init_data *data)
{
	...
	if (scp_protocol_reg_multi_read(SCP_PROTOCOL_CTRL_BYTE0, value, BYTE_FOUR))
		return -1;
	...
	if (scp_protocol_reg_multi_read(SCP_PROTOCOL_VSET_BOUNDARY_HBYTE,
		value, BYTE_FOUR))
		return -1;
