			...
			const char *buf, size_t count)
{
	...
	if (hisi_shell->is_framework != IS_FRAMEWORK)
		return -EINVAL;

	hisi_shell->temp = temp;
...
static int calc_shell_temp(struct hisi_shell_t *hisi_shell)
{
	...
	for (i = 0; i < hisi_shell->sensor_count; i++) {
		shell_sensor = (struct hisi_shell_sensor_t *)(uintptr_t)
			((u64)(uintptr_t)(hisi_shell->hisi_shell_sensor) +
			 (u64)((long)i) * (sizeof(struct hisi_shell_sensor_t) +
			 sizeof(struct hisi_temp_tracing_t) *
			 (u64)((long)hisi_shell->sample_count)));
		for (j = 0; j < hisi_shell->sample_count; j++) {
			k = (hisi_shell->index - j) <  0 ?
			     ((hisi_shell->index - j) + hisi_shell->sample_count) :
			     hisi_shell->index - j;
				  ...
				  int temp)
{
	...
	old_index = (hisi_shell->index - 1) <  0 ?
		     ((hisi_shell->index - 1) + hisi_shell->sample_count) :
		     hisi_shell->index - 1;
	...
	if (hisi_shell->valid_flag || hisi_shell->index) {
		if (shell_sensor->sensor_type & CHECK_TSENS &&
		    diff > hisi_shell->tsensor_temp_step) {
	...
	if (shell_sensor->sensor_type & CHECK_TSENS) {
		if (temp > hisi_shell->tsensor_max_temp ||
		    temp < hisi_shell->tsensor_min_temp) {
			pr_err("%s %s sensor_name = %s sensor_temp = %d "
			       "tsensor_temp_out_of_range\n", __func__,
			       hisi_shell->tz_dev->type,
			       shell_sensor->sensor_name, temp);
			shell_sensor->temp_tracing[hisi_shell->index].temp_invalid_flag =
				INVALID_TEMP_OUT_OF_RANGE;
			ret = HAVE_INVALID_SENSOR_TEMP;
		}
	} else if (shell_sensor->sensor_type & CHECK_NTC) {
		if (temp > hisi_shell->ntc_max_temp ||
		    temp < hisi_shell->ntc_min_temp) {
			pr_err("%s %s sensor_name = %s sensor_temp = %d "
...
static int calc_sensor_temp_avg(struct hisi_shell_t *hisi_shell, int *tsensor_avg,
				int *ntc_avg)
{
	int i;
	...
	for (i = 0; i < hisi_shell->sensor_count; i++) {
		...
		if (shell_sensor->sensor_type & CHECK_TSENS) {
			...
			if (!shell_sensor->temp_tracing[hisi_shell->index].temp_invalid_flag) {
				tsensor_good_count++;
				sum_tsensor_temp +=
					shell_sensor->temp_tracing[hisi_shell->index].temp;
			}
		}
	...
	for (i = 0; i < hisi_shell->sensor_count; i++) {
		...
		if (shell_sensor->sensor_type & CHECK_NTC &&
			...
			pr_err("%s, %s, handle ntc sensor %d invalid "
			       "data %d, result is %d\n", __func__,
			       hisi_shell->tz_dev->type, (i + 1),
			       invalid_temp, ntc_avg);
			shell_sensor->temp_tracing[hisi_shell->index].temp = ntc_avg;
		}
		trace_handle_invalid_temp(hisi_shell->tz_dev, i, invalid_temp,
					  shell_sensor->temp_tracing[hisi_shell->index].temp);
	}
...
static int
set_shell_temp(struct hisi_shell_t *hisi_shell, int have_invalid_temp, int index)
{
	int ret = 0;
	...
	if (hisi_shell->valid_flag && ret == 0) {
		hisi_shell->temp = calc_shell_temp(hisi_shell);
		if (hisi_shell->old_temp == 0) {
			hisi_shell->old_temp = hisi_shell->temp;
		} else {
			if ((hisi_shell->temp - hisi_shell->old_temp) >=
			    (hisi_shell->shell_temp_step_range *
			    (int)hisi_shell->interval / 1000))
				hisi_shell->temp = hisi_shell->old_temp +
			   		hisi_shell->shell_temp_step *
					(int)hisi_shell->interval / 1000;
			else if ((hisi_shell->temp - hisi_shell->old_temp) <=
				 (-hisi_shell->shell_temp_step_range *
				 (int)hisi_shell->interval / 1000))
				hisi_shell->temp = hisi_shell->old_temp -
					hisi_shell->shell_temp_step *
	...
	} else if (!hisi_shell->valid_flag && ret == 0) {
...
static void hkadc_sample_temp(struct work_struct *work)
{
	...
	hisi_shell =
		container_of((struct delayed_work *)work, struct hisi_shell_t, work); /*lint !e826*/
	...
	for (i = 0; i < hisi_shell->sensor_count ; i++) {
		...
		if (i == hisi_shell->channel - 1 && hisi_shell->debug_temp) {
			pr_err("%s, %s, sensor_name = %s, temp = %d\n", __func__,
			       hisi_shell->tz_dev->type,
			       shell_sensor->sensor_name,
			       hisi_shell->debug_temp);
			temp = hisi_shell->debug_temp;
		}
#endif

		shell_sensor->temp_tracing[hisi_shell->index].temp = temp;
		have_invalid_temp = hkadc_handle_temp_data(hisi_shell, shell_sensor,
							   (int)temp);
	}
...
static int fill_sensor_coef(struct hisi_shell_t *hisi_shell, struct device_node *np)
{
	...
	for_each_child_of_node(np, child) {
		shell_sensor = (struct hisi_shell_sensor_t *)(uintptr_t)
			((u64)(uintptr_t)hisi_shell +
			sizeof(struct hisi_shell_t) +
			(u64)((long)i) * (sizeof(struct hisi_shell_sensor_t) +
			sizeof(struct hisi_temp_tracing_t) *
			(u64)((long)hisi_shell->sample_count)));
		...
		/* terminal sensor */
			...
			pr_err("%s, %s, %s, sensor id get err\n", __func__,
			       hisi_shell->tz_dev->type, ptr_type);
...
static int get_shell_dts_para(struct device_node *dev_node, int sample_count,
			      struct hisi_shell_t *hisi_shell, int sensor_count,
			      struct device_node *np)
{
	...
	if (read_temp_range(dev_node, hisi_shell, tsensor_para,
...
static int create_framework_file_node(struct platform_device *pdev)
{
	...
	hisi_shell = platform_get_drvdata(pdev);
	hisi_shell->is_framework = is_framework;
	if (is_framework == IS_FRAMEWORK) {
		ret = create_file_node(pdev, &hisi_shell_attribute_group);
		if (ret == 0)
			pr_err("hisi shell framework node create ok!\n");
		return ret;
	}
	return -EINVAL;
}
...
static int hisi_shell_probe(struct platform_device *pdev)
{
	...
	hisi_shell = kzalloc(sizeof(struct hisi_shell_t) +
			     (u64)((long)sensor_count) *
			     (sizeof(struct hisi_shell_sensor_t) +
			     sizeof(struct hisi_temp_tracing_t) *
	...
	ret = get_shell_dts_para(dev_node, sample_count, hisi_shell,
				 sensor_count, np);
	if (ret != 0)
		goto free_mem;

	hisi_shell->tz_dev
		= thermal_zone_device_register(dev_node->name, 0, 0, hisi_shell,
