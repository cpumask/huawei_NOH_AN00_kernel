...
void emmc_get_rpmb_info(struct mmc_card *card, u8 *ext_csd)
{
	...
	/* according to EXT_CSD_RPMB_MULT, get the total units in rpmb and
	 * calculate the total blks */
	card->ext_csd.raw_rpmb_size_mult = ext_csd[EXT_CSD_RPMB_MULT];
	rpmb_config.rpmb_total_blks = (card->ext_csd.raw_rpmb_size_mult *
				       rpmb_config.rpmb_unit_size) >>
				      rpmb_config.rpmb_blk_size;
	...
	if (((card->ext_csd.raw_wr_rel_param >> 4) & 0x1) == 1)
		rpmb_config.rpmb_write_frame_support = ((uint64_t)1 << 31) |
						       ((uint64_t)1 << 1) |
						       ((uint64_t)1 << 0);
	else {
		rpmb_config.rpmb_write_frame_support =
			((uint64_t)1 << 1) | ((uint64_t)1 << 0);
		/* emmc 5.0, rpmb read need to align */
		rpmb_config.rpmb_read_align = 0x1;
	}
	/* according to EXT_CSD_VENDOR_FEATURE_SUPPORT bit 0 to set max rpmb
	 * frames */
	card->ext_csd.raw_vendor_feature_support =
		ext_csd[EXT_CSD_VENDOR_FEATURE_SUPPORT];
	...
	card->ext_csd.raw_rpmb_region1_size_mult =
		ext_csd[EXT_CSD_RPMB_REGION1_SIZE_MULT];
	card->ext_csd.raw_rpmb_region2_size_mult =
		ext_csd[EXT_CSD_RPMB_REGION2_SIZE_MULT];
	card->ext_csd.raw_rpmb_region3_size_mult =
		ext_csd[EXT_CSD_RPMB_REGION3_SIZE_MULT];
	card->ext_csd.raw_rpmb_region4_size_mult =
		ext_csd[EXT_CSD_RPMB_REGION4_SIZE_MULT];
	rpmb_config.rpmb_region_size[0] =
		card->ext_csd.raw_rpmb_region1_size_mult;
	rpmb_config.rpmb_region_size[1] =
		card->ext_csd.raw_rpmb_region2_size_mult;
	rpmb_config.rpmb_region_size[2] =
		card->ext_csd.raw_rpmb_region3_size_mult;
	rpmb_config.rpmb_region_size[3] =
		card->ext_csd.raw_rpmb_region4_size_mult;
	/* if emmc does not support multi key, we will set region 1 size is
	 * total units */
	if ((rpmb_config.rpmb_region_size[0] |
	     rpmb_config.rpmb_region_size[1] |
	     rpmb_config.rpmb_region_size[2] |
	     rpmb_config.rpmb_region_size[3]) == 0x0)
		rpmb_config.rpmb_region_size[0] =
