...
static void mmc_cmdq_task_info_init(struct mmc_card *card, struct request *req)
{
 ...
 */
static void mmc_blk_cmdq_setup_queue(struct mmc_queue *mq, struct mmc_card *card)
{
...
enum blk_eh_timer_return mmc_cmdq_rq_timed_out(struct request *req)
{
	...
	pr_err("%s: request with req: 0x%x, tag: %d, flags: 0x%x timed out\n",
		__func__, req, req->tag, req->cmd_flags);
