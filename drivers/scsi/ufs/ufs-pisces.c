...
static void ufs_ip_reset_for_hybrid(struct ufs_kirin_host *host)
{
	u32 reg;
...
void ufs_kirin_device_hw_reset(struct ufs_hba *hba)
{
	...
	if (!ufshcd_is_hisi_ufs_hc(hba)) {
		ufs_kirin_device_hw_reset_hybrid(hba);
		return;
	}
...
static void ana_eq_slicer_en(struct ufs_hba *hba)
{
	...
	hisi_uic_read_reg(hba, ATTR_MTX0(AFE_CONFIG_VIII), &value);
	/* BIT 0 is reg_afe_sl_en_eq */
	value |= BIT(0);
	hisi_uic_write_reg(hba, ATTR_MTX0(AFE_CONFIG_VIII), value);
	hisi_uic_write_reg(hba, ATTR_MTX1(AFE_CONFIG_VIII), value);
}
...
static void saveconfig_handle(struct ufs_hba *hba)
{
	 ...
	 * 250 * 250ns = 62.5us
	 */
	hisi_uic_write_reg(hba, VS_DDBUG_SAVE_CONFIG_TIME, SAVECFG_250NS);
	...
	struct ufs_hba *hba, struct ufs_pa_layer_attr *dev_req_params)
{
	...
	if (ufshcd_is_hisi_ufs_hc(hba)) {
		...
		ufs_hisi_kirin_pwr_change_pre_change(hba, dev_req_params);
		return;
	}
