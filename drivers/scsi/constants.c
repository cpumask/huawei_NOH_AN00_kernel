			 ...
			 const char **cdb_name, const char **sa_name)
{
	...
	*cdb_name = NULL;
	...
	if (opcode < ARRAY_SIZE(cdb_byte0_names))/*lint !e574*/
	...
	for (k = 0; k < arr_sz; ++k, ++arr) {/*lint !e644*/
...
scsi_extd_sense_format(unsigned char asc, unsigned char ascq, const char **fmt)
{
	...
	*fmt = NULL;
	for (i = 0; i < ARRAY_SIZE(additional); i++) {/*lint !e574*/
...
const char *scsi_hostbyte_string(int result)
{
	...
	if (hb < ARRAY_SIZE(hostbyte_table))/*lint !e574*/
...
const char *scsi_driverbyte_string(int result)
{
	...
	if (db < ARRAY_SIZE(driverbyte_table))/*lint !e574*/
...
const char *scsi_mlreturn_string(int result)
{
	...
	for (k = 0; k < ARRAY_SIZE(scsi_mlreturn_arr); ++k, ++arr) {/*lint !e574*/
		if (result == arr->value)
			return arr->name;
	}
	return NULL;
