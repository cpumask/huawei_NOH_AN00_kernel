...
static u32 hisi_secure_dw_readl(u32 base_addr, u32 offset)
{
...
static void hisi_secure_dw_writel(u32 base_addr, u32 offset, u32 b)
{
	struct arm_smccc_res res;
...
#if defined CONFIG_HISI_I2C_DESIGNWARE
	if (dev->secure_mode) {
		value = hisi_secure_dw_readl(dev->reg_base, offset);
	} else {
#endif
...
#if defined CONFIG_HISI_I2C_DESIGNWARE
	if (dev->secure_mode) {
		hisi_secure_dw_writel(dev->reg_base, offset, b);
	} else {
#endif
		if (dev->flags & ACCESS_16BIT) {
