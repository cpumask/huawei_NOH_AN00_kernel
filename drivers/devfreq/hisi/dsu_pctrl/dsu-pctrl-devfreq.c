...
static int dsu_pctrl_start_hwmon(struct perf_hwmon *hw)
{
	...
	cpuhp_setup_state_nocalls(CPUHP_AP_HISI_DSU_PCTRL_DEVFREQ_NOTIFY_PREPARE,
...
#else /* CONFIG_HISI_L3CACHE_EXTENSION_DYNAMIC */
	decide_enable_acp(dsu, *portions);
	dsu->cur_num_portions = *portions;
	portion_active = (1UL << dsu->cur_num_portions) - 1;
	set_portion_ctrl(portion_active);
#endif /* CONFIG_HISI_L3CACHE_EXTENSION_DYNAMIC */
	mutex_unlock(&g_l3c_acp_lock);
#else /* CONFIG_HISI_L3CACHE_SHARE_PERF */
	dsu->cur_num_portions = *portions;
	portion_active = (1UL << dsu->cur_num_portions) - 1;
	set_portion_ctrl(portion_active);
#endif /* CONFIG_HISI_L3CACHE_SHARE_PERF */
