...
/* 似有命令函数列表 */
OAL_STATIC OAL_CONST hut_cmd_entry_stru g_hut_cmd_ops[] = {
...
OAL_STATIC hut_mem_addr_stru g_base_addr;

/* HUT模块定义的tasklet(上报中断时使用) */
OAL_STATIC oal_tasklet_stru g_hut_tasklet;

/* 状态机(写内存命令会用到此状态机) */
OAL_STATIC hut_fsm_stru pa_hut_fsm;

/* 工作队列 */
OAL_STATIC hut_workqueue_stru g_hut_workqueue;

OAL_STATIC hut_cmd_fmt_stru g_tmp;

/* 中断队列，用于保存中断状态寄存器和错误状态寄存器 */
OAL_STATIC hut_intr_queue g_intr_queue;
...
OAL_STATIC oal_int32 hut_read_mem(oal_uint8 *puc_data)
{
    ...
    oal_spin_lock_bh(&g_hut_workqueue.st_spin_lock);

    pst_rx_node->puc_data = puc_data;
    oal_dlist_add_tail(&pst_rx_node->st_list, &g_hut_workqueue.st_list);

    oal_spin_unlock_bh(&g_hut_workqueue.st_spin_lock);

    return oal_workqueue_schedule(&g_hut_workqueue.rx_work);
}
...
OAL_STATIC oal_int32 hut_read_start_mem_addr(oal_uint8 *puc_data)
{
    ...
    /* 返回的是物理地址 */
    st_cmd_fmt.ul_addr = OAL_VIRT_TO_PHY_ADDR((oal_void *)g_base_addr.puc_base_addr_align);
...
OAL_STATIC oal_void hut_irq_isr_all(oal_void)
{
    ...
    /* 加锁，关中断 */
    oal_spin_lock_irq_save(&g_intr_queue.st_spin_lock, &ul_flag);
    ...
    oal_dlist_add_tail(&pst_rx_node->st_list, &g_intr_queue.st_list);

    /* 解锁，开中断 */
    oal_spin_unlock_irq_restore(&g_intr_queue.st_spin_lock, &ul_flag);

    /* 调度一个tasklet */
    oal_task_sched(&g_hut_tasklet);
}
...
oal_void hut_tasklet_handler(oal_uint l_data)
{
    ...
    while (!oal_dlist_is_empty(&g_intr_queue.st_list)) {
        pst_entry = g_intr_queue.st_list.pst_next;
...
OAL_STATIC OAL_INLINE oal_uint32 hut_get_cmd_id(oal_uint8 *puc_data, oal_uint8 *puc_cmd_id)
{
    ...
    for (uc_cmd_idx = 0; uc_cmd_idx < OAL_ARRAY_SIZE(g_hut_cmd_ops); uc_cmd_idx++) {
        /* 根据命令字查找命令ID */
        if (!oal_strcmp(g_hut_cmd_ops[uc_cmd_idx].pc_cmd_name, ac_cmd)) {
...
OAL_STATIC oal_uint32 hut_non_frag_msg_recv(oal_uint8 *puc_data, oal_uint32 ul_len)
{
    ...
    return (oal_uint32)g_hut_cmd_ops[uc_cmd_id].p_hut_cmd_func(puc_data + 2); /* 多偏移两个存放命令的字节 */
}
...
OAL_STATIC oal_int32 hut_2nd_frag_write_mem(oal_uint8 *puc_data, oal_void *p_param)
{
    ...
    if (memcpy_s((oal_uint8 *)ul_virt_addr, g_tmp.ul_val, puc_data + OAL_SIZEOF(hut_frag_hdr_stru), ul_len) != EOK) {
        HUT_ERR_VAR(0, "memcpy_s error, destlen=%u, srclen=%u\n ", g_tmp.ul_val, ul_len);
        return OAL_FAIL;
    }
...
oal_void hut_workqueu_handler(oal_work_stru *pst_work)
{
    ...
    while (!oal_dlist_is_empty(&g_hut_workqueue.st_list)) {
        pst_entry = g_hut_workqueue.st_list.pst_next;
        oal_dlist_delete_entry(pst_entry);
        oal_spin_unlock_bh(&g_hut_workqueue.st_spin_lock);
...
oal_int32 hut_main_init(oal_void)
{
    ...
    /* 记录内存分配的起始地址 */
    g_base_addr.puc_base_addr_origin = puc_base_addr;

    puc_base_addr = (oal_uint8 *)(uintptr_t)OAL_GET_4BYTE_ALIGN_VALUE((uintptr_t)puc_base_addr);

    /* 记录内存分配4字节对齐后的起始地址 */
    g_base_addr.puc_base_addr_align = puc_base_addr;

    HUT_INFO_VAR(0, "hut_main_init, mem start addr is 0x%x.",
                 OAL_VIRT_TO_PHY_ADDR((oal_void *)g_base_addr.puc_base_addr_align));

    OAL_INIT_WORK(&g_hut_workqueue.rx_work, hut_workqueu_handler);

    oal_dlist_init_head(&g_hut_workqueue.st_list);
    oal_spin_lock_init(&g_hut_workqueue.st_spin_lock);

    oal_dlist_init_head(&g_intr_queue.st_list);
    oal_spin_lock_init(&g_intr_queue.st_spin_lock);

    /* 向WAL模块注册HUT netlink接收函数 */
    oam_netlink_ops_register(OAM_NL_CMD_HUT, hut_receive_msg);

    /* 初始化HUT模块tasklet */
    oal_task_init(&g_hut_tasklet, hut_tasklet_handler, 0);
...
{
    if (g_base_addr.puc_base_addr_origin != OAL_PTR_NULL) {
        /* 释放申请的内存 */
        oal_free(g_base_addr.puc_base_addr_origin);
    }

    /* 卸载HUT.KO加载时注册的中断服务程序，并将运行模式置为NORMAL */
    hal_to_hut_irq_isr_unregister();

    oal_cancel_work_sync(&g_hut_workqueue.rx_work);
