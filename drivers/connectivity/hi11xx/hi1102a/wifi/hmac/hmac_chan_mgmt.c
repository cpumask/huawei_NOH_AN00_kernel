                                                  ...
                                                  wlan_channel_bandwidth_enum_uint8 *pen_new_bandwidth)
{
    ...
    pst_chan_scan_report = (hmac_eval_scan_report_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL, uc_max_supp_channle *
    ...
    if (mac_device_is_auto_chan_sel_enabled(pst_mac_device) == OAL_FALSE) {
        if (mac_is_dbac_enabled(pst_mac_device)) {
            ...
            ul_ret = mac_get_channel_idx_from_num(pst_mac_vap->st_channel.en_band,
                                                  hmac_chan_get_user_pref_primary_ch(pst_mac_device),
                                                  &uc_user_chan_idx);
        }
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                             "{chan_select_channel_for_operation::mac_get_channel_idx_from_num failed[%d].}", ul_ret);
            OAL_MEM_FREE((oal_void *)pst_chan_scan_report, OAL_TRUE);
