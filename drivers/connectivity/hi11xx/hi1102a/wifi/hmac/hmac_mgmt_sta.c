...
oal_uint32 hmac_sta_wait_asoc(hmac_vap_stru *pst_sta, oal_void *pst_msg)
{
    ...
    FRW_TIMER_CREATE_TIMER(&(pst_sta->st_mgmt_timer), hmac_mgmt_timeout_sta,
                           pst_hmac_asoc_req->us_assoc_timeout, &(pst_sta->st_mgmt_timetout_param),
                           OAL_FALSE, OAM_MODULE_ID_HMAC, pst_sta->st_vap_base_info.ul_core_id);
...
#ifdef _PRE_WLAN_FEATURE_LOCATION_RAM
            if (oal_memcmp(puc_data + 2, g_auc_huawei_oui, MAC_OUI_LEN) == 0) {
...
oal_uint8 *hmac_sta_find_ie_in_probe_rsp(mac_vap_stru *pst_mac_vap, oal_uint8 uc_eid, oal_uint16 *pus_index)
{
    ...
    if (pst_scanned_bss_info == OAL_PTR_NULL) {
        OAM_WARNING_LOG4(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                         ...
                         pst_mac_vap->auc_bssid[0], pst_mac_vap->auc_bssid[3],
...
OAL_STATIC oal_uint32 hmac_sta_sync_bss_freq(hmac_vap_stru *pst_hmac_vap, mac_channel_stru *pst_channel,
                                             wlan_protocol_enum_uint8 en_protocol)
{
    if (hmac_ap_clean_bss(pst_hmac_vap) == OAL_SUCC) {
...
OAL_STATIC oal_void hmac_sta_sync_bss_freq_all(hmac_vap_stru *pst_hmac_sta)
{
    ...
    for (uc_idx = 0; uc_idx < pst_mac_dev->uc_vap_num; uc_idx++) {
        ...
        if ((pst_hmac_vap->st_vap_base_info.st_channel.en_band == pst_channel->en_band) &&
            ...
            if (hmac_sta_sync_bss_freq(pst_hmac_vap, pst_channel, pst_hmac_sta->st_vap_base_info.en_protocol) !=
                OAL_SUCC) {
                OAM_WARNING_LOG1(0, OAM_SF_ASSOC, "{hmac_sta_sync_bss_freq_all::sync vap%d failed}",
