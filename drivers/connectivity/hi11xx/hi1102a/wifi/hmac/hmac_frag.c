                                  ...
                                  oal_netbuf_stru *pst_netbuf, mac_tx_ctl_stru *pst_tx_ctl)
{
    ...
    if (pst_hmac_user->st_user_base_info.st_key_info.en_cipher_type == WLAN_80211_CIPHER_SUITE_TKIP) {
        uc_last_frag = OAL_NETBUF_LEN(pst_netbuf) % (ul_threshold - (oal_uint32)uc_ic_header -
                                                     pst_tx_ctl->uc_frame_header_length);
        if ((uc_last_frag > 0) && (uc_last_frag <= 8)) { /* TKIP加密时，最后一个分片payload长度小于等于8时，无法进行加密 */
            ul_threshold = ul_threshold + 8; /* 调整分片门限 */
    ...
    if (pst_hmac_user->pst_defrag_netbuf == OAL_PTR_NULL) {
        ...
        if (pst_mac_device == OAL_PTR_NULL) {
            OAM_ERROR_LOG4(pst_hmac_user->st_user_base_info.uc_vap_id, OAM_SF_ANY,
                           ...
                           pst_hmac_user->st_user_base_info.auc_user_mac_addr[3], /* auc_user_mac_addr第3byte输出打印 */
                           pst_hmac_user->st_user_base_info.auc_user_mac_addr[4], /* auc_user_mac_addr第4byte输出打印 */
                           pst_hmac_user->st_user_base_info.auc_user_mac_addr[5]); /* auc_user_mac_addr第5byte输出打印 */
            /* user异常，丢弃报文 */
            oal_netbuf_free(pst_netbuf);
            OAM_STAT_VAP_INCR(pst_hmac_user->st_user_base_info.uc_vap_id, rx_defrag_process_dropped, 1);
            return OAL_PTR_NULL;
