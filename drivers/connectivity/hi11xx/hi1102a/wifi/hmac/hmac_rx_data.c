 ...
 */
    ...
    if (pst_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
        ...
        /* 将MSDU链表交给发送流程处理 */
        if ((oal_netbuf_list_empty(&st_w2w_netbuf_hdr) == OAL_FALSE) &&
            (oal_netbuf_tail(&st_w2w_netbuf_hdr) != OAL_PTR_NULL) &&
...
oal_void hmac_rx_process_data_ap_tcp_ack_opt(hmac_vap_stru *pst_vap, oal_netbuf_head_stru *pst_netbuf_header)
{
    ...
    while (us_netbuf_num != 0) {
        ...
        /* 没有找到对应的用户 */
        if (ul_rslt != OAL_SUCC) {
            OAM_INFO_LOG0(st_event_hdr.uc_vap_id, OAM_SF_RX, "{hmac_rx_lan_frame_classify:frame is a unique frame.}");
        ...
        if (pst_hmac_da_user == OAL_PTR_NULL) {
            OAM_WARNING_LOG1(st_event_hdr.uc_vap_id, OAM_SF_RX, "{hmac_rx_lan_frame_classify::hmac_da_user[%d] null.}",
                             us_user_dix);
            OAM_STAT_VAP_INCR(pst_vap->st_vap_base_info.uc_vap_id, rx_da_check_dropped, 1);
