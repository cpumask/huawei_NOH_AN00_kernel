...
oal_uint32 hmac_amsdu_tx_process(hmac_vap_stru *pst_vap, hmac_user_stru *pst_user, oal_netbuf_stru *pst_buf)
{
    ...
    if (pst_amsdu->uc_msdu_num == 0) {
        ...
        /* 启动定时器 */
        FRW_TIMER_CREATE_TIMER(&pst_amsdu->st_amsdu_timer, hmac_amsdu_tx_timeout_process,
                                       ...
                                       oal_netbuf_stru *pst_buf, mac_tx_ctl_stru *pst_tx_ctl)
{
    ...
    if (g_tx_large_amsdu.uc_cur_amsdu_ampdu_enable[pst_hmac_vap->st_vap_base_info.uc_vap_id] == OAL_FALSE) {
        return;
    }
