...
oal_bool_enum_uint8 mac_is_ftm_req_frame(oal_netbuf_stru *pst_netbuf)
{
    ...
    /* Management frame */
    if (mac_get_frame_sub_type(puc_mac_header) == (WLAN_FC0_SUBTYPE_ACTION | WLAN_FC0_TYPE_MGT)) {
...
oal_bool_enum_uint8 mac_is_ftm_frame(oal_netbuf_stru *pst_netbuf)
{
    ...
    /* Management frame */
    if ((mac_get_frame_type(puc_mac_header) == WLAN_FC0_TYPE_MGT) &&
        (mac_get_frame_sub_type(puc_mac_header) == WLAN_FC0_SUBTYPE_ACTION)) {
...
oal_void mac_set_ft_ie(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint16 *pus_ie_len)
{
    ...
    *pus_ie_len = 84; /* ft ie的 Element ID、 Length、MIC Control、MIC、ANonceSNonce共占84字节 */
}
...
oal_void mac_set_wfa_tpc_report_ie(oal_void *pst_vap, oal_uint8 *puc_buffer, oal_uint8 *puc_ie_len)
{
    ...
    if (memcpy_s(puc_buffer + uc_index, MAC_OUI_LEN, g_auc_wfa_oui, MAC_OUI_LEN) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "mac_set_wfa_tpc_report_ie::memcpy fail!");
    }
    uc_index += MAC_OUI_LEN;
