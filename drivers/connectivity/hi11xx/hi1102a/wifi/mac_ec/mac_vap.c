...
{
#ifdef _PRE_WLAN_FEATURE_MULTI_NETBUF_AMSDU
    g_tx_large_amsdu.uc_cur_amsdu_ampdu_enable[pst_mac_vap->uc_vap_id] = OAL_FALSE;
#endif
}
...
oal_void mac_vap_vowifi_init(mac_vap_stru *pst_mac_vap)
{
    ...
    if (pst_mac_vap->pst_vowifi_cfg_param == OAL_PTR_NULL) {
        pst_mac_vap->pst_vowifi_cfg_param =
            OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL, OAL_SIZEOF(mac_vowifi_param_stru), OAL_TRUE);
        if (pst_mac_vap->pst_vowifi_cfg_param == OAL_PTR_NULL) {
