    ...
    switch (en_type) {
        ...
        case NL80211_IFTYPE_ADHOC:
            OAM_ERROR_LOG1(0, OAM_SF_CFG,
                           "{wal_cfg80211_change_virtual_intf::currently we do not support this type[%d]}", en_type);
            return -OAL_EINVAL;
...
#endif
            {
                pst_net_dev->ieee80211_ptr->iftype = en_type;  // P2P BUG P2P_DEVICE 提前创建，不需要通过wpa_supplicant 创建
                                                   ...
                                                   oal_station_info_stru *pst_stats)
{
    ...
    if ((hitalk_status != 0) && (-85 > pst_sta_info->signal)) {
...
#endif
{
...
#else
    pst_net_dev = oal_net_alloc_netdev(OAL_SIZEOF(oal_netdev_priv_stru), auc_name,
                                       oal_ether_setup); /* 此函数第一个入参代表私有长度，添加NAPI后修改 */
#endif
    if (OAL_UNLIKELY(pst_net_dev == OAL_PTR_NULL)) {
    ...
    pst_wdev = (oal_wireless_dev_stru *)OAL_MEM_ALLOC(OAL_MEM_POOL_ID_LOCAL, OAL_SIZEOF(oal_wireless_dev_stru),
                                                      OAL_FALSE);
