...
/* 函数实现 */
oal_void oam_stats_report_irq_info_to_sdt(oal_uint8 *puc_irq_info_addr, oal_uint16 us_irq_info_len)
{
    oal_uint32 ul_tick;
 ...
 */
oal_uint32 oam_stats_report_timer_info_to_sdt(oal_uint8 *puc_timer_addr, oal_uint8 uc_timer_len)
{
...
oal_uint32 oam_stats_report_info_to_sdt(oam_ota_type_enum_uint8 en_ota_type)
{
    ...
    if (en_ota_type == OAM_OTA_TYPE_DEV_STAT_INFO) {
        ul_ret = memcpy_s((oal_void *)pst_ota_data->auc_ota_data,
                          (oal_uint32)pst_ota_data->st_ota_hdr.us_ota_data_len,
                          (const oal_void *)g_oam_stat_info.ast_dev_stat_info,
                          (oal_uint32)us_stat_info_len);
    } else {
        ul_ret = memcpy_s((oal_void *)pst_ota_data->auc_ota_data,
                          (oal_uint32)pst_ota_data->st_ota_hdr.us_ota_data_len,
                          (const oal_void *)g_oam_stat_info.ast_vap_stat_info,
                          (oal_uint32)us_stat_info_len);
    }
    if (ul_ret != EOK) {
        oal_mem_sdt_netbuf_free(pst_netbuf, OAL_TRUE);
...
oal_uint32 oam_stats_report_usr_info(oal_uint16 us_usr_id)
{
    ...
    ul_ret = memcpy_s((oal_void *)pst_ota_data->auc_ota_data,
                      (oal_uint32)pst_ota_data->st_ota_hdr.us_ota_data_len,
                      (const oal_void *)&g_oam_stat_info.ast_user_stat_info[us_usr_id],
                      (oal_uint32)us_stat_info_len);
    if (ul_ret != EOK) {
        oal_mem_sdt_netbuf_free(pst_netbuf, OAL_TRUE);
...
{
    memset_s(&g_oam_stat_info, OAL_SIZEOF(oam_stat_info_stru), 0, OAL_SIZEOF(oam_stat_info_stru));
}
...
oal_uint32 oam_stats_clear_vap_stat_info(oal_uint8 uc_vap_id)
{
    ...
    memset_s(&g_oam_stat_info.ast_vap_stat_info[uc_vap_id], OAL_SIZEOF(oam_vap_stat_info_stru),
...
oal_uint32 oam_stats_clear_user_stat_info(oal_uint16 us_usr_id)
{
    ...
    memset_s(&g_oam_stat_info.ast_user_stat_info[us_usr_id], OAL_SIZEOF(oam_user_stat_info_stru),
