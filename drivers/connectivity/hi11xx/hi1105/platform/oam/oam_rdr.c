...
#include "oam_rdr.h"
#include "securec.h"
#if (defined CONFIG_HISI_BB) && (defined CONFIG_HI110X_PLAT_BB) && !defined(CONFIG_HI110X_KERNEL_MODULES_BUILD_SUPPORT)
...
STATIC int32 g_init_done = 0;
STATIC struct work_struct g_hisi_conn_dump_work;
STATIC struct workqueue_struct *g_hisi_conn_rdr_wq;
STATIC struct hisi_conn_info_s g_hisi_conn_info;

STATIC int32 hisi_conn_stat_init(void)
{
    int32 i;
    struct timex txc;
    do_gettimeofday(&(txc.time));
    txc.time.tv_sec -= sys_tz.tz_minuteswest * 60; /* sec是minute*60 */
...
STATIC void _hisi_conn_rdr_system_error(uint32 modid, uint32 arg1, uint32 arg2)
{
