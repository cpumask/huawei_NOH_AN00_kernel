...
// 每个被跟踪的内存块的信息
typedef struct {
    uintptr_t ul_addr;                // 内存块地址
    uint32_t ul_time;               // 内存申请时间戳
    uint32_t ul_fileid;             // 申请该内存块的文件ID
    uint32_t ul_linenum;            // 申请该内存块的代码行号
    uint32_t ul_last_trace_fileid;  // 该内存块最近一次的跟踪点
    uint32_t ul_last_trace_line;    // 该内存块最近一次的跟踪点
    uint32_t ul_last_trace_time;    // 该内存块最近一次的跟踪时间戳
...
typedef struct {
    ...
    uint32_t ul_cnt;
    uint32_t ul_node_shortage;  // 发生过node不够用的情形
} mem_trace_mgmt;
