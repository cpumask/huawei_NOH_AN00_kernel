...
static ssize_t guard_thermal_store(struct device_driver *drv, const char *buf, size_t count)
{
    ...
    if (count > 1) {
        errno_t err = memset_s(s_cfgdev.sbuf, sizeof(s_cfgdev.sbuf), 0, sizeof(s_cfgdev.sbuf));
        if (err != EOK) {
            cam_warn("%s memset_s fail", __func__);
        }
        if (count <= sizeof(s_cfgdev.sbuf)) {
            err = memcpy_s(s_cfgdev.sbuf, sizeof(s_cfgdev.sbuf) - 1, buf, count - 1);
            if (err != EOK) {
                cam_warn("%s mcmcpy fail", __func__);
            }
        } else {
            size_t sbuf_size = sizeof(s_cfgdev.sbuf);
            err = memcpy_s(s_cfgdev.sbuf, sbuf_size - 1, buf, sbuf_size - 1);
            if (err != EOK) {
                cam_warn("%s mcmcpy fail", __func__);
            }
...
static ssize_t guard_thermal_show(struct device_driver *drv, char *buf)
{
    ...
    cam_info("%s enter", __func__);
    ret = snprintf_s(offset, PAGE_SIZE, PAGE_SIZE - 1, "guard thermal:[%s]\n", s_cfgdev.sbuf);
    if (ret < 0) {
...
static ssize_t hwcam_log_show(struct device_driver *drv, char *buf)
{
    int ret = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "0x%08x\n", hwcam_debug_mask);
    if (ret < 0) {
        cam_warn("%s snprintf_s fail", __func__);
    }
    return ret;
...
hwcam_cfgdev_thermal_guard(struct v4l2_event* ev)
{
    ...
    if (buf != NULL) {
        errno_t err = memcpy_s(buf, sizeof(s_cfgdev.sbuf), s_cfgdev.sbuf, sizeof(s_cfgdev.sbuf));
        if (err != EOK) {
        ...
        struct v4l2_subdev* sd,hwcam_device_id_constants_t dev_const)
{
    ...
    if (rc < 0) {
        ...
        (void)snprintf_s(vdev->name, sizeof(vdev->name), sizeof(vdev->name) - 1, "%s", media_prefix);
