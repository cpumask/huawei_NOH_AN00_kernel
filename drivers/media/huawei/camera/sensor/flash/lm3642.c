...
static int g_flash_arry[16] = { 94, 188, 281, 375, 469, 563, 656, 750,
	844, 938, 1031, 1125, 1219, 1313, 1406, 1500
};
static int g_torch_arry[8] = { 48, 94, 141, 188, 234, 281, 328, 375 };

/* Internal varible define */
static struct hw_lm3642_private_data_t g_lm3642_pdata;
static struct hw_flash_ctrl_t g_lm3642_ctrl;
static struct i2c_driver g_lm3642_i2c_driver;
	...
	struct hw_flash_cfg_data *cdata)
{
	...
	struct hw_lm3642_private_data_t *pdata = NULL;
	unsigned char val;
	int current_level = 0;
	int rc = 0;
	...
	loge_if_ret(i2c_func->i2c_write(i2c_client, REG_FLASH_FEATURES,
		FLASH_RAMP_TIME | FLASH_TIMEOUT) < 0);
	...
	struct device_attribute *attr, char *buf)
{
	...
	rc = snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE,
		...
		g_lm3642_ctrl.state.mode, g_lm3642_ctrl.state.mode);
	if (rc <= 0) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cdata.mode == STANDBY_MODE) {
		rc = hw_lm3642_off(&g_lm3642_ctrl);
		if (rc < 0) {
		...
		cdata.data = LM3642_TORCH_MAX_RT_CUR;
		cam_info("%s mode=%d, max_current=%d", __func__,
			cdata.mode, cdata.data);

		rc = hw_lm3642_on(&g_lm3642_ctrl, &cdata);
		if (rc < 0) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	rc = snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE,
		...
		g_lm3642_ctrl.flash_mask_enable);
	if (rc <= 0) {
		cam_err("%s snprintf_s failed", __func__);
		return -EINVAL;
	}
...
{
	if (buf[0] == '0')
		g_lm3642_ctrl.flash_mask_enable = false;
	else
		g_lm3642_ctrl.flash_mask_enable = true;
...
#ifdef CAMERA_FLASH_FACTORY_TEST
static struct device_attribute g_lm3642_lightness =
	__ATTR(flash_lightness, 0664, hw_lm3642_lightness_show,
	hw_lm3642_lightness_store);
#endif

static struct device_attribute g_lm3642_flash_mask =
	__ATTR(flash_mask, 0664, hw_lm3642_flash_mask_show,
	hw_lm3642_flash_mask_store);

static int hw_lm3642_register_attribute(struct hw_flash_ctrl_t *flash_ctrl,
	struct device *dev)
{
	int rc;
...
#ifdef CAMERA_FLASH_FACTORY_TEST
	rc = device_create_file(dev, &g_lm3642_lightness);
	if (rc < 0) {
		cam_err("%s failed to creat lightness attribute", __func__);
		goto err_create_lightness_file;
	}
#endif
	rc = device_create_file(dev, &g_lm3642_flash_mask);
	if (rc < 0) {
...
#ifdef CAMERA_FLASH_FACTORY_TEST
	device_remove_file(dev, &g_lm3642_lightness);
#endif
...
#ifdef CAMERA_FLASH_FACTORY_TEST
	register_camerafs_attr(&g_lm3642_lightness);
#endif
	return 0;
}
...
static int hw_lm3642_remove(struct i2c_client *client)
{
	...
	g_lm3642_ctrl.func_tbl->flash_exit(&g_lm3642_ctrl);
...
static const struct i2c_device_id g_lm3642_id[] = {
	{ "lm3642", (unsigned long) &g_lm3642_ctrl },
	{}
};

static const struct of_device_id g_lm3642_dt_match[] = {
...
static struct i2c_driver g_lm3642_i2c_driver = {
	...
	.id_table = g_lm3642_id,
	.driver = {
		.name = "hw_lm3642",
		.of_match_table = g_lm3642_dt_match,
	},
};
...
{
	cam_info("%s erter\n", __func__);
	return i2c_add_driver(&g_lm3642_i2c_driver);
}

static void __exit hw_lm3642_module_exit(void)
{
	cam_info("%s enter", __func__);
	i2c_del_driver(&g_lm3642_i2c_driver);
}

static struct hw_flash_i2c_client g_lm3642_i2c_client;

static struct hw_flash_fn_t g_lm3642_func_tbl = {
...
static struct hw_flash_ctrl_t g_lm3642_ctrl = {
	.flash_i2c_client = &g_lm3642_i2c_client,
	.func_tbl = &g_lm3642_func_tbl,
	.hw_flash_mutex = &flash_mut_lm3642,
	.pdata = (void *)&g_lm3642_pdata,
