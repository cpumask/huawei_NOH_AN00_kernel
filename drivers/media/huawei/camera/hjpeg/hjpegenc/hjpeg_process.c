...
static int hjpeg_poweron_ippcomm(hjpeg_base_t* pJpegDev);
static int hjpeg_poweroff_ippcomm(hjpeg_base_t* pJpegDev);
...
static int is_du_allocate_update = DU_ALLOCATE_UPDATE_NO;
static int is_read_twice = 0;
...
static int __check_buffer_vaild(int share_fd, unsigned int req_addr, unsigned int req_size)
{
	...
	if (s_hjpeg.hw_ctl.smmu_type == ST_SMMUV3) {
		...
		ret = hicam_buf_v3_map_iommu(pdev, share_fd, &fmt, s_hjpeg.hw_ctl.support_iova_padding);
		if (ret < 0) {
	...
	if (s_hjpeg.hw_ctl.smmu_type == ST_SMMUV3) {
		hicam_buf_v3_unmap_iommu(pdev, share_fd, &fmt, s_hjpeg.hw_ctl.support_iova_padding);
	} else {
...
void dump_reg(void)
{
	...
	if (is_irq_merge) {
		...
		cam_info("%s: top reg: 0x%08x=0x%08x", __func__, JPG_RO_STATE,
				get_reg_val((void __iomem*)((char*)viraddr + JPG_RO_STATE)));
		cam_info("%s: top reg: 0x%08x=0x%08x", __func__, JPGENC_CRG_CFG0,
				get_reg_val((void __iomem*)((char*)viraddr + JPGENC_CRG_CFG0)));
		cam_info("%s: top reg: 0x%08x=0x%08x", __func__, JPGENC_CRG_CFG1,
				get_reg_val((void __iomem*)((char*)viraddr + JPGENC_CRG_CFG1)));
		cam_info("%s: top reg: 0x%08x=0x%08x", __func__, JPGENC_MEM_CFG,
				get_reg_val((void __iomem*)((char*)viraddr + JPGENC_MEM_CFG)));
	}
...
static int hjpeg_poweron_ippcomm(hjpeg_base_t* pjpeg_dev)
{
	...
	if (s_hjpeg_ipp_comm == NULL) {
		cam_err("%s: s_hjpeg_ipp_comm is null!", __func__);
		return EINVAL;
	}

	if (atomic_read(&pjpeg_dev->jpeg_on)) {
	...
	mutex_unlock(&pjpeg_dev->wake_lock_mutex); /*lint !e456 */
	...
	if ((ret = hjpeg_setclk_enable(pjpeg_dev, JPEG_FUNC_CLK)) != 0) {
		cam_err("Failed: hjpeg_setclk_enable.%d\n", ret);
		if ((ret = s_hjpeg_ipp_comm->power_dn(s_hjpeg_ipp_comm)) != 0)
			cam_err("Failed: ippcomm power dn.%d\n", ret);
		goto FAILED_JPG_POWERON;
	}
...
static int get_dts_power_prop(struct device *pdev)
{
	...
	}else if (PC_IPPCOMM == s_hjpeg.hw_ctl.power_controller) {
		ret = of_property_read_u32(np, "jpegclk-rate-mode", &(s_hjpeg.hw_ctl.jpgclk_prop.jpegclk_mode));
		if (ret != 0) {
			cam_info("[%s] Failded: of_property_read_u32 jpegclk-rate-mode", __func__, s_hjpeg.hw_ctl.jpgclk_prop.jpegclk_mode);
			goto error;
		}

		ret = of_property_read_u32(np, "jpegclk-lowfreq-mode", &(s_hjpeg.hw_ctl.jpgclk_prop.jpegclk_lowfreq_mode));
		if (ret != 0) {
			cam_info("[%s] Failded: of_property_read_u32 jpegclk-rate-mode", __func__, s_hjpeg.hw_ctl.jpgclk_prop.jpegclk_lowfreq_mode);
			goto error;
		}

		ret = of_property_read_u32(np, "jpegclk-offfreq-mode", &(s_hjpeg.hw_ctl.jpgclk_prop.jpegclk_offreq_mode));
		if (ret != 0) {
			cam_info("[%s] Failded: of_property_read_u32 jpegclk-rate-mode", __func__, s_hjpeg.hw_ctl.jpgclk_prop.jpegclk_offreq_mode);
			goto error;
		}
...
static int get_dts_secadapt_prop(struct device *pdev)
{
	...
	cam_info("%s %d: start_swid: %u swid_num: %u", __func__, __LINE__, base_array[0], base_array[1]);
...
static int get_dts_cvdr_prop(struct device *pdev)
{
	...
	//is_cvdr_dis_timeout
	ret = of_property_read_u32(np, "huawei,cvdr_dis_timeout", &cvdr_dis_timeout);
	if (ret < 0)
		cam_info("%s: get cvdr_dis_timeout flag failed.", __func__);
	cam_info("%s: cvdr_dis_timeout=<%d>", __func__, cvdr_dis_timeout);

	// is_du_allocate_update
    ret = of_property_read_u32(np, "huawei,du_allocate_update", &is_du_allocate_update);
    if (ret < 0) {
        cam_info("%s: get is_du_allocate_update failed.", __func__);
    }
...
static int hjpeg_get_dts(struct platform_device* pDev)
{
	...
	// iova-padding-mode
	ret = of_property_read_u32(np, "huawei,support-iova-padding", &(s_hjpeg.hw_ctl.support_iova_padding));
	if (ret == 0)
		cam_info("%s: support-iova-padding=%u", __func__, s_hjpeg.hw_ctl.support_iova_padding);

	ret = of_property_read_u32(np, "huawei,twice_read_reg", &is_read_twice);
	if (ret == 0)
		cam_info("%s: is_read_twice=%u", __func__, is_read_twice);
...
static int hjpeg_setclk_enable(hjpeg_base_t* pJpegDev, int idx)
{
	...
	if (PC_IPPCOMM == pJpegDev->hw_ctl.power_controller) {
		ret = s_hjpeg_ipp_comm->set_jpgclk_rate(s_hjpeg_ipp_comm, pJpegDev->hw_ctl.jpgclk_prop.jpegclk_mode);
		if (ret != 0) {
			cam_err("[%s] Failed: ipp_comm->set_jpgclk_rate %u .%d\n",
					__func__, pJpegDev->hw_ctl.jpgclk_prop.jpegclk_mode, ret);
			ret = s_hjpeg_ipp_comm->set_jpgclk_rate(s_hjpeg_ipp_comm, pJpegDev->hw_ctl.jpgclk_prop.jpegclk_lowfreq_mode);
			if (ret != 0)
				cam_err("[%s] Failed: ipp_comm->set_jpgclk_rate %u .%d\n",
						__func__, pJpegDev->hw_ctl.jpgclk_prop.jpegclk_mode, ret);
		}
...
static void hjpeg_setclk_disable(hjpeg_base_t* pJpegDev, int idx)
{
	...
	if (PC_IPPCOMM == pJpegDev->hw_ctl.power_controller) {
		ret = s_hjpeg_ipp_comm->set_jpgclk_rate(s_hjpeg_ipp_comm, pJpegDev->hw_ctl.jpgclk_prop.jpegclk_offreq_mode);
		if (ret != 0)
			cam_err("[%s] Failed: ipp_comm->set_jpgclk_rate off: %d, ret: %d\n", __func__,
				pJpegDev->hw_ctl.jpgclk_prop.jpegclk_offreq_mode, ret);
		return;
	}
...
static int hjpeg_clk_ctrl(void __iomem* subctrl1, bool enable)
{
	...
	if (is_read_twice != 0) {
		cur_clk = get_reg_val(subctrl_clk);
		cam_info("%s(%d) read again status 0x%x", __func__, __LINE__, cur_clk);
	}
...
static void hjpeg_apbbus_timeout_ctrl(void __iomem* subctrl1)
{
	...
	if (cvdr_dis_timeout){
		//disable APB bus timeout,bit19/20,should be 0, default val is 0x00120001
		set_reg_val((void __iomem*)((char*)top_cfg0_addr), 0x00020001);
		cam_info("%s: IPP_TOP reg: 0x%08x=0x%08x", __func__, DMA_CRG_CFG0,
...
static void hjpeg_encode_finish(void __iomem* jpegenc, void __iomem* subctl)
{
    ...
    switch (is_irq_merge) {
        case JPEG_IRQ_IN_OUT:
            ...
            (void)get_reg_val((void __iomem*)((char*)subctl + JPGENC_IRQ_REG2));
        ...
        struct platform_device* pdev )
{
	...
	if (s_hjpeg.hw_ctl.power_controller == PC_IPPCOMM) {
		s_hjpeg_ipp_comm = hipp_common_register(HISI_JPEGE_UNIT, &(pdev->dev));
		if (s_hjpeg_ipp_comm == NULL) {
...
{
	cam_debug("%s: is_du_allocate_update=%d", __func__, is_du_allocate_update);
	return is_du_allocate_update;
}
...
hjpeg_exit_module(void)
{
	...
	if (s_hjpeg.hw_ctl.power_controller == PC_IPPCOMM) {
		if (hipp_common_unregister(HISI_JPEGE_UNIT) != 0)
