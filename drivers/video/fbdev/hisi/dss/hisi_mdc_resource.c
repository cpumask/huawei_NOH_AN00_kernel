...
static int mdc_check_mmbuf_size_available(struct hisi_fb_data_type *hisifd, mdc_ch_info_t *chn_info,
...
static int mdc_chn_pre_request(struct hisi_fb_data_type *hisifd, mdc_ch_info_t *chn_info,
	mdc_func_ops_t *mdc_ops, uint32_t mmbuf_mdc_reserved_size)
{
	...
	for (i = 0; i < mdc_ops->chan_num; i++) {
		...
		if (mdc_check_mmbuf_size_available(hisifd, chn_info, mmbuf_mdc_reserved_size))
			return -EINVAL;
...
static int mdc_chn_request(struct hisi_fb_data_type *hisifd, mdc_ch_info_t *chn_info,
	mdc_func_ops_t *mdc_ops, uint32_t mmbuf_mdc_reserved_size)
{
	unsigned int i;
	mdc_chn_info_t *mdc_chn = NULL;
...
static int mdc_chn_request_handle(struct hisi_fb_data_type *hisifd, mdc_ch_info_t *chn_info)
{
	int ret;
	mdc_func_ops_t *mdc_ops = NULL;
	uint32_t mmbuf_mdc_reserved_size;

	hisi_check_and_return((!hisifd || !chn_info), -EINVAL, ERR, "hisifd or chn_info is NULL\n");
	...
	ret = mdc_chn_pre_request(hisifd, chn_info, mdc_ops, mmbuf_mdc_reserved_size);
	if (ret != -EBUSY)
		return ret;

	ret = mdc_chn_request(hisifd, chn_info, mdc_ops, mmbuf_mdc_reserved_size);
	if (ret != -EBUSY)
		return ret;
...
int hisi_mdc_chn_request(struct fb_info *info, void __user *argp)
{
	...
	hisi_check_and_return(!info, -EINVAL, ERR, "info NULL pointer!\n");

	hisifd = (struct hisi_fb_data_type *)info->par;
	hisi_check_and_return(!hisifd, -EINVAL, ERR, "hisifd NULL pointer!\n");

	if (hisifd->index != AUXILIARY_PANEL_IDX) {
		HISI_FB_INFO("fb%d don't support\n", hisifd->index);
		return -EINVAL;
	}
	...
	hisi_check_and_return((ret != 0), -EINVAL, ERR, "fb%d, copy for user failed! ret=%d\n", hisifd->index, ret);
