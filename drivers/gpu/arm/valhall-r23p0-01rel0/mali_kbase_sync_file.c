...
kbase_sync_fence_out_trigger(struct kbase_jd_atom *katom, int result)
{
	...
	if ((katom->kctx->kbdev->hisi_dev_data.game_pid == katom->kctx->tgid) &&
		katom->ai_freq_flag)
		katom->kctx->kbdev->hisi_dev_data.signaled_seqno++;
#endif
...
#endif
{
...
#endif
	{
		...
		/* dependenced fence is fault because of context dying */
#if (KERNEL_VERSION(4, 11, 0) <= LINUX_VERSION_CODE || \
	 (KERNEL_VERSION(4, 10, 0) > LINUX_VERSION_CODE && \
	  KERNEL_VERSION(4, 9, 68) <= LINUX_VERSION_CODE))
		if (kcb->fence->error == -255)
#else
		if (kcb->fence->status == -255)
#endif
			katom->dying_dependence_fault = true;
	}
...
void kbase_sync_fence_in_cancel_wait(struct kbase_jd_atom *katom)
{
	if (!kbase_fence_free_callbacks(katom)) {
		...
		dev_err(katom->kctx->kbdev->dev,
			"kbase_sync_fence_in_cancel_wait kctx %pK atom %pK wait couldn't be cancelled\n",
			(void *)katom->kctx, katom);
		return;
	}

	dev_err(katom->kctx->kbdev->dev,
			"kbase_sync_fence_in_cancel_wait kctx %pK atom %pK wait was cancelled\n",
			(void *)katom->kctx, katom);
