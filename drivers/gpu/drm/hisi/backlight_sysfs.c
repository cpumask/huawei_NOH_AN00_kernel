 ...
 *
 */
/*lint -e86 -e421 -e546 -e548 -e574 -e580*/
...
{
	int ret;
	ret = memset_s(&bd->fb_notif, sizeof(bd->fb_notif), 0, sizeof(bd->fb_notif));
	drm_check_and_return(ret != EOK, -ENOMEM, ERR, "memset for notif failed!");
...
{
	struct backlight_device *new_bd = NULL;
	int rc;
	int ret;

	pr_debug("backlight device register: name=%s\n", name);
	...
	if (props) {
		ret = memcpy_s(&new_bd->props, sizeof(struct backlight_properties), props,
		       sizeof(struct backlight_properties));
		if (ret != EOK) {
			kfree(new_bd);
			new_bd = NULL;
			HISI_DRM_ERR("get props failed!");
			return ERR_PTR(-ENOMEM);
		}
...
struct backlight_device *devm_backlight_device_register(struct device *dev,
	...
	const struct backlight_properties *props)
{
	struct backlight_device **ptr = NULL;
