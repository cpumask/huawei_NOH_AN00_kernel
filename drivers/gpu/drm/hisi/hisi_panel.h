...
struct hisi_panel_info {
	...
	struct dss_sharpness_bit *sharp2d_table;
	...
	/*
	 * for mipi dsi tx interface, support delayed cmd queue
	 * which will send after next frame start(vsync)
	 */
...
extern struct mipi_ifbc_division g_mipi_ifbc_division[MIPI_DPHY_NUM][IFBC_TYPE_MAX];

int hisi_blpwm_on(struct hisi_panel_info *panel_info);
int hisi_blpwm_off(struct hisi_panel_info *panel_info);
int hisi_blpwm_set_backlight(
	struct hisi_panel_info *panel_info, uint32_t bl_level);
void hisi_blpwm_set_brightness(uint32_t brightness);

bool is_mipi_video_panel(struct hisi_panel_info *panel_info);
bool is_mipi_cmd_panel(struct hisi_panel_info *panel_info);
