...
#ifndef LINUX_LM36923_H
#define LINUX_LM36923_H
...
#define LM36923_EMERG(msg, ...)    \
	do { if (lm36923_msg_level > 0)  \
		pr_emerg("[lm36923]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
#define LM36923_ALERT(msg, ...)    \
	do { if (lm36923_msg_level > 1)  \
		pr_alert("[lm36923]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
#define LM36923_CRIT(msg, ...)    \
	do { if (lm36923_msg_level > 2)  \
		pr_crit("[lm36923]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
#define LM36923_ERR(msg, ...)    \
	do { if (lm36923_msg_level > 3)  \
		pr_err("[lm36923]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
#define LM36923_WARNING(msg, ...)    \
	do { if (lm36923_msg_level > 4)  \
		pr_warn("[lm36923]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
#define LM36923_NOTICE(msg, ...)    \
	do { if (lm36923_msg_level > 5)  \
		pr_info("[lm36923]%s: "msg, __func__, ## __VA_ARGS__); } while (0)
