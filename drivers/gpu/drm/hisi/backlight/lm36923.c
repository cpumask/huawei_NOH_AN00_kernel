...
static void set_brt_ctrl_reg(struct device_node *np, struct lm36923_chip_data *pchip)
{
	int ret;
...
static void get_lm36923_led_num(struct device_node *np, int *enable_reg)
{
	int ret;
	...
	if (ret < 0) {
		...
		*enable_reg = 0xF;
		...
		case TI_LM36923_LED_TWO:
			*enable_reg = 0x7;
			break;
		case TI_LM36923_LED_THREE: /* fall-throuth */
		default:
			*enable_reg = 0xF;
			break;
		}
		g_reg_val[2] = *enable_reg;
		LM36923_INFO("lm36923 led enable  =  0x%d\n", *enable_reg);
	}
}

static int lm36923_disable_reg_config(struct device_node *np, struct lm36923_chip_data *pchip)
{
	int ret;
...
static int set_boost_ctrl_reg(struct device_node *np, struct lm36923_chip_data *pchip)
{
	int ret;
...
static int set_fault_ctrl_reg(struct device_node *np, struct lm36923_chip_data *pchip)
{
	int ret;
	int fault_ctrl = 0;
...
static int lm36923_chip_init(struct lm36923_chip_data *pchip)
{
	...
	set_brt_ctrl_reg(np, pchip);

	get_lm36923_led_num(np, &enable_reg);
	...
	ret = lm36923_disable_reg_config(np, pchip);
	if (ret < 0)
		goto out;

	ret = set_fault_ctrl_reg(np, pchip);
	if (ret < 0)
		goto out;

	ret = of_property_read_u32(np, "lm36923_hw_en_gpio", &lm36923_hw_en_gpio);
	if (ret < 0)
 ...
 * be returned in error cases.
 */
static void enable_reg_for_fake_lcd(uint32_t level, int *enable_flag, int *disable_flag)
{
	if (g_fake_lcd_flag) {
		if (level > 0) {
			if (*enable_flag == 0) {
				regmap_write(g_pchip->regmap, REG_ENABLE, lcd_fake_panel_enable_reg);
			...
			*disable_flag = 0;
			...
			if (*disable_flag == 0) {
				regmap_write(g_pchip->regmap, REG_ENABLE, 0x0);
...
ssize_t lm36923_set_backlight_reg(uint32_t bl_level)
{
	...
	enable_reg_for_fake_lcd(level, &enable_flag, &disable_flag);
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (dev == NULL)
		return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "dev is null\n");

	pchip = dev_get_drvdata(dev);
	if (pchip == NULL)
		return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "data is null\n");

	client = pchip->client;
	if (client == NULL)
		return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "client is null\n");

	ret = regmap_read(pchip->regmap, REG_BRT_CTR, &mode);
	if (ret < 0)
		return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "LM36923 I2C read error\n");

	return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "LM36923 bl_mode:0x%x\n", mode);
}
...
i2c_error:
	...
	return snprintf_s((char *)buf, PAGE_SIZE, PAGE_SIZE - 1, "%s: i2c access fail to register\n", __func__);

out_input:
	dev_err(pchip->dev, "%s:input conversion fail\n", __func__);
	return snprintf_s((char *)buf, PAGE_SIZE, PAGE_SIZE - 1, "%s: input conversion fail\n", __func__);
}

static DEVICE_ATTR(bled_mode, 0644, lm36923_bled_mode_show, lm36923_bled_mode_store);
				...
				struct device_attribute *attr, char *buf)
{
	...
	return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "LM36923 bl_level:%d\n", bl_level);
}
				...
				struct device_attribute *attr, char *buf)
{
	...
	return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "Revision(0x00)= 0x%x\nSoftware Reset(0x01)= 0x%x\n \
...
static void get_led_num(int *result, struct lm36923_chip_data *pchip)
{
	int led_num;
	int i;
...
static int probe_init(struct i2c_client *client, struct i2c_adapter *adapter,
...
static int create_dev_for_lm36923(struct lm36923_chip_data *pchip, struct i2c_client *client)
{
	int ret = 0;
...
#if defined(CONFIG_HUAWEI_DSM)
static int dsm_config(struct lm36923_chip_data *pchip, struct i2c_client *client)
{
	unsigned int val = 0;
	int ret;
				...
				const struct i2c_device_id *id)
{
	...
	ret = probe_init(client, adapter, &pchip);
	if (ret)
		return ret;
#ifdef CONFIG_REGMAP_I2C
...
#if defined(CONFIG_HUAWEI_DSM)
	ret = dsm_config(pchip, client);
	if (ret < 0)
		goto err_out;
#endif

	ret = create_dev_for_lm36923(pchip, client);
	if (ret)
		goto err_sysfs;
