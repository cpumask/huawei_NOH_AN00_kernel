...
static int delta_x_0;
static int delta_y_0;
static int delta_x_1;
static int delta_y_1;
static int target_x;
static int target_y;
static int ghost_algo3_num;

void ts_kit_algo_det_ght_init(void)
{
	TS_LOG_INFO("%s:%s init ghost resource\n", __func__, GHOST_LOG_TAG);
	memset(&finger_touch, 0, sizeof(finger_touch));
	memset(&pre_finger_touch, 0, sizeof(pre_finger_touch));
}
...
static void ts_algo_det_ght_finger_release(void)
{
	...
	do_gettimeofday(&tv);
	if (finger_touch.finger_event == TS_FINGER_PRESS) {
		TS_LOG_DEBUG("%s:%s finger release\n", __func__, GHOST_LOG_TAG);
		finger_touch.finger_event = TS_FINGER_RELEASE;
		memcpy(&(finger_touch.finger_release_tv), &tv, sizeof(tv));
	}
}
...
static void ts_algo_det_ght_finger_press(int index, int x, int y)
{
	...
	if (!(finger_touch.finger_num_flag & (1 << index))) {
		TS_LOG_DEBUG("%s: %s record finger_num_flag: %d\n", __func__,
			GHOST_LOG_TAG, finger_touch.finger_num_flag);
		finger_touch.x[index] = x;
		finger_touch.y[index] = y;
		finger_touch.finger_num_flag |= (1 << index);
		memcpy(&(finger_touch.finger_press_tv[index]), &tv, sizeof(tv));
	}
}
...
static int ts_detect_ghost_algo1(int index)
{
	...
	delta_time = ((finger_touch.finger_release_tv.tv_sec -
		finger_touch.finger_press_tv[index].tv_sec) *
		GHOST_MIL_SECOND_TIME) +
		finger_touch.finger_release_tv.tv_usec -
		finger_touch.finger_press_tv[index].tv_usec;
	if ((delta_time > 0) && (delta_time < FINGER_PRESS_TIME_MIN_ALGO1)) {
		if (!ghost_time_record[index]) {
			ghost_time_record[index] =
				finger_touch.finger_release_tv.tv_sec;
			ghost_num_record_per_second[index]++;
		} else if ((ghost_time_record[index] ==
				finger_touch.finger_release_tv.tv_sec) ||
				(ghost_time_record[index] ==
				(finger_touch.finger_release_tv.tv_sec -
				GHOST_OPERATE_ONE_SECOND))) {
			ghost_num_record_per_second[index]++;
		} else {
		...
		TS_LOG_DEBUG("%s:%s ghost_num_record_per_second:%d\n", __func__,
			GHOST_LOG_TAG, ghost_num_record_per_second[index]);
		if (ghost_num_record_per_second[index] >
			FINGER_PRESS_TIMES_ALGO1) {
			ghost_num_record_per_second[index] = 0;
			ghost_time_record[index] = 0;
			TS_LOG_INFO("%s:%s DETECT:%s\n", __func__,
				GHOST_LOG_TAG, GHOST_REASON_OPERATE_TOO_FAST);
...
/*
 * If there are FINGER_PRESS_TIMES_ALGO2
 * consecutive down/up operations,
 * the distance of x or y axis coordinate between each
 * operation is less than GHOST_OPER_DISTANCE_ALGO2 pixe,
...
static int ts_detect_ghost_algo2(int index)
{
	...
	if ((delta_x < GHOST_OPER_DISTANCE_ALGO2) &&
		(delta_x > (SET_TO_NEGATIVE_NUM * GHOST_OPER_DISTANCE_ALGO2)))
		ghost_num_record_x[index]++;
	else
		ghost_num_record_x[index] = 0;

	if ((delta_y < GHOST_OPER_DISTANCE_ALGO2) &&
		(delta_y > (SET_TO_NEGATIVE_NUM * GHOST_OPER_DISTANCE_ALGO2)))
		ghost_num_record_y[index]++;
	else
		ghost_num_record_y[index] = 0;

	TS_LOG_DEBUG(
		"%s:%s ghost_num_record_x[%d]:%d ghost_num_record_y[%d]:%d\n",
		__func__, GHOST_LOG_TAG, index, ghost_num_record_x[index],
		index, ghost_num_record_y[index]);

	if ((ghost_num_record_x[index] > FINGER_PRESS_TIMES_ALGO2) ||
		(ghost_num_record_y[index] > FINGER_PRESS_TIMES_ALGO2)) {
		TS_LOG_INFO("%s:%s DETECT:%d\n", __func__, GHOST_LOG_TAG,
			GHOST_OPERATE_IN_XY_AXIS);
		ghost_num_record_x[index] = 0;
		ghost_num_record_y[index] = 0;
...
/*
 * If we detect two fingers down,the interval time is
 * less than FINGER_PRESS_TIME_MIN_ALGO3,
 * One of the fingers is always in the same
 * position(the distance is less than GHOST_OPER_DISTANCE_ALGO3)
 * and the user operate consecutive FINGER_PRESS_TIMES_ALGO3 times,
 * we think this should not a human being operate.
 */
static int ts_detect_ghost_algo3(int finger_num)
{
	...
	if ((finger_num == GHOST_OPERATE_TWO_FINGERS) &&
		(pre_finger_touch.finger_num_flag > GHOST_OPERATE_ONE_FINGERS)) {
		if (target_x == 0) {
			delta_time = ((finger_touch.finger_press_tv[1].tv_sec -
				finger_touch.finger_press_tv[0].tv_sec) *
				GHOST_MIL_SECOND_TIME) +
				finger_touch.finger_press_tv[1].tv_sec -
				finger_touch.finger_press_tv[0].tv_sec;
			if (delta_time < FINGER_PRESS_TIME_MIN_ALGO3)
				for (i = 0; i < GHOST_OPERATE_TWO_FINGERS; i++)
					for (j = 0; j < GHOST_OPERATE_TWO_FINGERS; j++) {
						...
						if ((delta_x < GHOST_OPER_DISTANCE_ALGO3) &&
							(delta_x > SET_TO_NEGATIVE_NUM * GHOST_OPER_DISTANCE_ALGO3) &&
							(delta_y < GHOST_OPER_DISTANCE_ALGO3) &&
							(delta_y > SET_TO_NEGATIVE_NUM * GHOST_OPER_DISTANCE_ALGO3)) {
			...
			/* Calculate distance between first and target point */
			delta_x_0 = finger_touch.x[0] - target_x;
			delta_y_0 = finger_touch.y[0] - target_y;
			/* Calculate distance between second and target point */
			...
			if ((delta_x_0 < GHOST_OPER_DISTANCE_ALGO3) &&
				(delta_x_0 > SET_TO_NEGATIVE_NUM *
				GHOST_OPER_DISTANCE_ALGO3) &&
				(delta_time < FINGER_PRESS_TIME_MIN_ALGO3) &&
				(delta_y_0 < GHOST_OPER_DISTANCE_ALGO3) &&
				(delta_y_0 > SET_TO_NEGATIVE_NUM *
				GHOST_OPER_DISTANCE_ALGO3)) {
				ghost_algo3_num++;
			} else if ((delta_x_1 < GHOST_OPER_DISTANCE_ALGO3) &&
				(delta_x_1 > SET_TO_NEGATIVE_NUM *
				GHOST_OPER_DISTANCE_ALGO3) &&
				(delta_time < FINGER_PRESS_TIME_MIN_ALGO3) &&
				(delta_y_1 < GHOST_OPER_DISTANCE_ALGO3) &&
				(delta_y_1 > SET_TO_NEGATIVE_NUM *
				GHOST_OPER_DISTANCE_ALGO3)) {
				ghost_algo3_num++;
			} else {
				ghost_algo3_num = 0;
			...
			TS_LOG_DEBUG("%s:%s ghost_algo3_num:%d\n", __func__,
				GHOST_LOG_TAG, ghost_algo3_num);

			if (ghost_algo3_num == FINGER_PRESS_TIMES_ALGO3) {
				TS_LOG_INFO("%s:%s DETECT:%d\n", __func__,
					GHOST_LOG_TAG, GHOST_OPERATE_IN_SAME_POSITION);
				target_x = 0;
				target_y = 0;
				ghost_algo3_num = 0;
...
static void ts_algo_detect_ghost(int finger_cnt)
{
	...
	if (finger_touch.finger_event != TS_FINGER_RELEASE) {
	...
	TS_LOG_DEBUG("%s:%s finger_num_flag:%d finger_num:%d\n", __func__,
		GHOST_LOG_TAG, finger_touch.finger_num_flag, finger_num);
	...
	if (finger_touch.finger_num_flag) {
		for (index = 0; index < TS_MAX_FINGER; index++) {
			...
			if (finger_touch.finger_num_flag & (1 << index)) {
				ghost_detect_flag |=
					ts_detect_ghost_algo1(index);
				if (pre_finger_touch.finger_num_flag & (1 << index))
					ghost_detect_flag |=
						ts_detect_ghost_algo2(index);
			}
		}
		...
		memcpy(&pre_finger_touch, &finger_touch,
			sizeof(pre_finger_touch));
		memset(&finger_touch, 0, sizeof(finger_touch));
	} else {
		TS_LOG_DEBUG("touch release, but not detect any touch press\n");
	}
	TS_LOG_DEBUG("%s -\n", __func__);
}
	...
	for (index = 0, id = 0; index < TS_MAX_FINGER; index++, id++) {
		if (in_info->cur_finger_number == 0) {
			...
			if ((in_info->fingers[index].x != 0) ||
				...
				if (dev_data && dev_data->ghost_detect_support)
					ts_algo_det_ght_finger_press(index,
					out_info->fingers[id].x,
					out_info->fingers[id].y);
			} else {
				out_info->fingers[id].status = 0;
