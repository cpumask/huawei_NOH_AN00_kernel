...
int getChannelsLength(void)
{
	...
	if (ret < OK) {
		TS_LOG_ERR( "%s getChannelsLength: ERROR %08X\n", __func__, ret);
...
int getFrameData(u16 address, int size, short *frame)
{
	...
	if (data == NULL) {
		TS_LOG_ERR( "%s getFrameData: ERROR %08X\n", __func__, ERROR_ALLOC);
		return ERROR_ALLOC;
	}
...
int getMSFrame3(MSFrameType type, MutualSenseFrame *frame)
{
	u16 offset = 0;
	int ret = 0;
	int  force_len = getForceLen();
	int sense_len = getSenseLen();
	...
	TS_LOG_INFO( "%s: Starting to get frame %02X\n", __func__,type);
	switch (type) {
	case MS_RAW:
...
LOAD_NORM:
		if (force_len == 0 || sense_len == 0) {
			TS_LOG_ERR(" %s: number of channels not initialized ERROR %08X\n", __func__, ERROR_CH_LEN);
...
LOAD_FRC:
	...
	TS_LOG_INFO( "%s: Force_len = %d Sense_len = %d Offset = %04X\n", __func__, force_len, sense_len, offset);
	...
	memset(frame->node_data, 0, frame->node_data_size
				* sizeof(short));
...
{
	u16 offset_force = 0, offset_sense = 0;
	int ret = 0;
	...
	TS_LOG_INFO("%s: Force_len = %d Sense_len = %d Offset_force = %04X Offset_sense = %04X\n",
	...
	if (frame->force_data == NULL) {
		TS_LOG_ERR( "%s: can not allocate force_data ERROR %08X\n",  __func__, ERROR_ALLOC | ERROR_GET_FRAME);
		return ERROR_ALLOC | ERROR_GET_FRAME;
	}
	memset(frame->force_data, 0, frame->header.force_node *
							 sizeof(short));
	...
	if (frame->sense_data == NULL) {
		...
		frame->force_data = NULL;
		TS_LOG_ERR( "%s: can not allocate sense_data ERROR %08X\n", __func__, ERROR_ALLOC | ERROR_GET_FRAME);
		return ERROR_ALLOC | ERROR_GET_FRAME;
	}
	memset(frame->sense_data, 0, frame->header.sense_node *
					     sizeof(short));
	ret = getFrameData(offset_force, frame->header.force_node *
			   BYTES_PER_NODE, (frame->force_data));
	if (ret < OK) {
		TS_LOG_ERR( "%s: error while reading force data ERROR %08X\n",__func__, ERROR_GET_FRAME_DATA);
		kfree(frame->force_data);
		frame->force_data = NULL;
	...
	if (ret < OK) {
		TS_LOG_ERR("%s: error while reading sense data ERROR %08X\n", __func__, ERROR_GET_FRAME_DATA);
		kfree(frame->force_data);
		frame->force_data = NULL;
		kfree(frame->sense_data);
		frame->sense_data = NULL;
		       ...
		       u64 *address)
{
	...
	u8 data[SYNCFRAME_DATA_HEADER] = {0};
	int ret = 0;

	if(!msHeader || !ssHeader || !address){
		TS_LOG_ERR( "%s:input error!\n", __func__);
		return ERROR_OP_NOT_ALLOW;
	}
...
int getSyncFrame(u8 type, MutualSenseFrame *msFrame, SelfSenseFrame *ssFrame)
{
	...
	TS_LOG_INFO( "%s: Starting to get Sync Frame %02X...\n", __func__, type);
	switch (type) {
	...
	TS_LOG_INFO( "%s: Requesting Sync Frame %02X...\n", __func__, type);
	res = requestSyncFrame(type);
	if (res < OK) {
		TS_LOG_ERR("%s: error while requesting Sync Frame ERROR %08X\n",
	...
	if (msFrame->node_data == NULL) {
		TS_LOG_ERR("%s: impossible allocate memory for MS frame... ERROR %08X\n",
			 __func__, ERROR_ALLOC | ERROR_GET_FRAME);
		return ERROR_ALLOC | ERROR_GET_FRAME;
	}
	...
	if (ssFrame->force_data == NULL) {
		TS_LOG_ERR("%s: impossible allocate memory for SS force frame...ERROR %08X\n",
			  __func__, ERROR_ALLOC | ERROR_GET_FRAME);
	...
	if (ssFrame->sense_data == NULL) {
		TS_LOG_ERR("%s: impossible allocate memory for SS sense frame...ERROR %08X\n",
			 __func__, ERROR_ALLOC | ERROR_GET_FRAME);
		res = ERROR_ALLOC | ERROR_GET_FRAME;
		goto ERROR;
	}
...
ERROR:
	if (res < OK) {
		...
		TS_LOG_INFO( "Getting Sync Frame FAILED! ERROR %08X!\n", res);
	} else {
		TS_LOG_INFO( "Getting Sync Frame FINISHED!\n");
