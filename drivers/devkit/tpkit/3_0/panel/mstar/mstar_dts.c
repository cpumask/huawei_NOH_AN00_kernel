...
int mstar_parse_dts(struct device_node *device, struct ts_kit_device_data *chip_data)
{
    ...
    retval =
        of_property_read_u8(device, "charger_supported",
    ...
    dts_val = 0;
    retval = of_property_read_u32(device, MSTAR_SUPPORT_GET_TP_COLOR, &dts_val);
    if (!retval) {
    	tskit_mstar_data->support_get_tp_color = (bool)dts_val;
        ...
        tskit_mstar_data->support_get_tp_color = 0; // default 0: no need know tp color
 		TS_LOG_ERR("%s, get device support get tp color failed, will use default value: 0 \n ", __func__);
	}
	...
	TS_LOG_INFO("%s:mstar_parse_dts succ\n", __func__);
    return NO_ERR;
}
