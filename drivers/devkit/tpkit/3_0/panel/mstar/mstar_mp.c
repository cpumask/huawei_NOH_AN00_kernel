...
#define RATIO_OF_OPEN_VALUE 100
#define RATIO_OF_OPEN_VALUE_UP 10000
#define MSG2846A_VACUT 13
#define MSG28XXA_VACUT 12
#define MSG_DEF_VACUT 15

#define MP_TEST_RESET_DELAY 100
#define CHECK_MP_SWITCH_DELAY 20
#define MSTAR_DRV_NUM 6
#define MSG2846A_DRV_NUM 7
int run_type = 0;
...
static void mstar_get_vacut_data(int *va_cutdata)
{
	if (!va_cutdata) {
		TS_LOG_ERR("vacut is NULL\n", __func__);
		return;
	}

	switch (tskit_mstar_data->mstar_chip_data->ic_type) {
	case CHIP_TYPE_MSG2846A:
		*va_cutdata = MSG2846A_VACUT;
		break;
	case CHIP_TYPE_MSG28XXA:
		*va_cutdata = MSG28XXA_VACUT;
		break;
	default:
		TS_LOG_ERR("unknown ic_type, set va_cut as default:15\n");
		*va_cutdata = MSG_DEF_VACUT;
		break;
	}

	TS_LOG_INFO("%s: ic_type = 0x%x, va_cutdata = %d\n", __func__,
		tskit_mstar_data->mstar_chip_data->ic_type, *va_cutdata);
}
...
static int new_send_test_cmd(u16 fmode, int mode)
{
    ...
    u16 chargeT, dumpT;
    int postidle;
	unsigned int ic_type = tskit_mstar_data->mstar_chip_data->ic_type;
    ...
    {
        case TYPE_SHORT:
	if ((ic_type == CHIP_TYPE_MSG2846A) ||
		(ic_type == CHIP_TYPE_MSG28XXA)) {
...
static int new_flow_start_test(void)
{
    ...
    if(tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG2846A){
		...
		if (mp_test_data->run_type != RUN_SHORT_TEST) {
			TS_LOG_INFO("save open data for uniformity test\n");
			for (i = 0; i < deltac_size; i++)
				mp_test_result->pdeltac_buffer[i] = deltac_data[i];
		}
    }
...
static void mp_calc_golden_range(int *goldenTbl, u16 weight, u16 golden_up, int *maxTbl, int *minTbl, int length)
{
	...
	if ((tskit_mstar_data->mstar_chip_data->ic_type ==
		CHIP_TYPE_MSG2846A) || (tskit_mstar_data->chip_type ==
		CHIP_TYPE_MSG28XXA)) {
		...
		for (i = 0; i < allnode_number; i++) {
			value = (int)weight * abs(goldenTbl[i]) /
				RATIO_OF_OPEN_VALUE;
			value_up = (int)weight_up * abs(goldenTbl[i]) /
				RATIO_OF_OPEN_VALUE_UP;
...
static int mp_main_init_var(void)
{
	...
	if(tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG2846A)
		mp_test_result->pborder_ratio_buf = kcalloc(MSG2846A_DRV_NUM *
			mp_test_data->sensorInfo.numSen, sizeof(int), GFP_KERNEL);
	else
		mp_test_result->pborder_ratio_buf = kcalloc(MSTAR_DRV_NUM *
			mp_test_data->sensorInfo.numSen, sizeof(int), GFP_KERNEL);
	...
	if (ERR_ALLOC_MEM(mp_test_result->puniformity_deltac) ||
		ERR_ALLOC_MEM(mp_test_result->pdelta_LR_buf) ||
		ERR_ALLOC_MEM(mp_test_result->pdelta_UD_buf) ||
...
static int mp_start_test(void)
{
    ...
    if(mp_test_data->UIConfig.bUniformityTest == 1)
    {
        for(retry = 0; retry < MP_RETRY_TEST_TIME; retry++)
        {
			...
			if (tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG2846A)
				mp_test_result->nUniformityResult =
					mp_new_flow_main(RUN_UNIFORMITY_TEST);
			else
				mp_test_result->nUniformityResult =
					p_mp_func->uniformity_judge(mp_test_result->pdeltac_buffer,
						MAX_MUTUAL_NUM);
