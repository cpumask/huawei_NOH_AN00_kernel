...
#define RING_BUFFER_SIZE 		(61440) 									/* accoring to 6402 ring_buffer size, which will be totally transmited to AP */
...
#define HI6402_FAST_TRAN_COUNT 		(RING_BUFFER_SIZE / (FAST_FRAME_LENGTH * VALID_BYTE_COUNT) + FAST_HEAD_FRAME_COUNT + FAST_TAIL_FRAME_COUNT)	/* 16 + 2=18 */
#define FAST_TRAN_COUNT 		(RING_BUFFER_SIZE / (FAST_FRAME_LENGTH * VALID_BYTE_COUNT_FAST) + FAST_HEAD_FRAME_COUNT + FAST_TAIL_FRAME_COUNT)	/* 8 + 2=10 */
...
static void pcm_fast_buffer_check(struct fast_tran_info *fast_info)
{
	...
	uint32_t fast_valid_buffer_len = RING_BUFFER_SIZE / VALID_BYTE_COUNT;
...
static int32_t dma_start(struct st_fast_status * fast_status)
{
	...
	logi("dma start request pll resource and switch\n");
	if (ret != 0) {
...
static int32_t dma_open(struct st_fast_status * fast_status)
{
	...
	/*get normal buffer*/
	static_ring_buffer_init(normal_info->normal_buffer, (RINGBUF_FRAME_LEN * VALID_BYTE_COUNT), RINGBUF_FRAME_COUNT);
...
static int32_t dma_get_max_read_len(enum codec_hifi_type codec_type, size_t *max_read_len, size_t count)
{
	if (CODEC_HI6402 == codec_type) {
		*max_read_len = (RING_BUFFER_SIZE > (hi6402_normal_frame_length * VALID_BYTE_COUNT)) ?
			RING_BUFFER_SIZE : (hi6402_normal_frame_length * VALID_BYTE_COUNT);/*lint !e647*/
	} else if (CODEC_HI6403 == codec_type) {
		*max_read_len = (RING_BUFFER_SIZE > (HI6403_NORMAL_FRAME_LENGTH * VALID_BYTE_COUNT)) ?
			RING_BUFFER_SIZE : (HI6403_NORMAL_FRAME_LENGTH * VALID_BYTE_COUNT);
	}
#ifdef CONFIG_SND_SOC_HI6405
	else if (CODEC_HI6405 == codec_type) {
		*max_read_len = (RING_BUFFER_SIZE > (HI6405_NORMAL_FRAME_LENGTH * VALID_BYTE_COUNT)) ?
			RING_BUFFER_SIZE : (HI6405_NORMAL_FRAME_LENGTH * VALID_BYTE_COUNT);
	}
#endif
	else {
		loge("type = %d invalid .\n", codec_type);
...
static ssize_t dma_fops_read(struct file *file, char __user *buffer, size_t count, loff_t *f_ops)
{
	...
	/* second, hal already read fast data, we copy normal data to user */
		if (!static_ring_buffer_is_empty()) {
			...
			static_ring_buffer_get(pcm_buffer);
	...
	if (dma_drv_info->st_normal_tran_info.read_count_left >= dma_drv_info->st_normal_tran_info.irq_count_left) {
		...
		if (dma_drv_info->st_normal_tran_info.normal_frame_find_flag == FRAME_NOT_FIND) {
			if (pcm_start_addr_find(SOUNDTRIGGER_PCM_NORMAL, ring_buf, RINGBUF_FRAME_LEN, &start_addr)) {
				static_ring_buffer_put(ring_buf);
...
{
	if (!g_dma_drv_info) {
		loge("device not init, use default config .\n");
		return -ENOENT;
	}
