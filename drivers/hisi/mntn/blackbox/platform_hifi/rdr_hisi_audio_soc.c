...
#ifdef HIFI_RESET_CDEV
		hifi_reset_inform_modem(HIFI_RESET_STATE_OFF);
#else
...
#ifdef HIFI_RESET_CDEV
	hifi_reset_inform_modem(HIFI_RESET_STATE_READY);
#else
...
{
	BB_PRINT_PN("%s():socdsp watchdog irq arrival\n", __func__);
...
#ifdef HIFI_RESET_CDEV
	ret = hifi_reset_dev_init();
	if (ret) {
		BB_PRINT_ERR("hifi reset dev init failed! return 0x%x\n", ret);
		goto dev_init_error;
	}
#endif
