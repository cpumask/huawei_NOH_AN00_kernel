...
static int hifi_dsp_wakeup_wired_write_thread(unsigned long arg)
{
	(void)arg;

	spin_lock_bh(&s_misc_data.para_write_lock);
	s_misc_data.para_write_wait_flag = true;
	spin_unlock_bh(&s_misc_data.para_write_lock);

	wake_up(&s_misc_data.para_write_waitq);
							...
							unsigned long arg)
{
	...
	case HIFI_MISC_IOCTL_WAKEUP_PARA_WRITE_THREAD: /*lint !e30 !e142*/
		logi("ioctl: HIFI_MISC_IOCTL_WAKEUP_PARA_WRITE_THREAD\n");
		mutex_lock(&s_misc_data.ioctl_mutex);
		ret = hifi_dsp_wakeup_wired_write_thread((uintptr_t)data32);
		mutex_unlock(&s_misc_data.ioctl_mutex);
		break;
...
static ssize_t hifi_misc_wired_para_write(struct file *file, const char __user *buf,
	size_t count, loff_t *ppos)
{
	int ret;
	...
	if (count == 0 || count > HIFI_PARA_DOWNLOAD_BUFFER_SIZE) {
		loge("param err, count = %zu\n", count);
		return -EINVAL;
	}

	ret = wait_event_interruptible(s_misc_data.para_write_waitq, s_misc_data.para_write_wait_flag); /*lint !e40 !e578 !e712 !e774 !e845*/
	if (ret) {
	...
	spin_lock_bh(&s_misc_data.para_write_lock);
	s_misc_data.para_write_wait_flag = false;
	spin_unlock_bh(&s_misc_data.para_write_lock);

	if (try_copy_from_user(s_misc_data.hifi_priv_base_virt + (HIFI_WIRED_HEADSET_PARA_ADDR -
		HIFI_UNSEC_BASE_ADDR), buf, count)) {
...
static const struct file_operations hifi_volume_meter_fops = {
	.owner = THIS_MODULE, /*lint !e64*/
	.write = hifi_misc_wired_para_write,
}; /*lint !e785*/
...
static void hifi_misc_proc_init( void )
{
	...
	struct proc_dir_entry *entry_hifi_volume_meter = NULL;
	...
	entry_hifi_volume_meter = proc_create("hifi_volume_meter", 0440, hifi_misc_dir,
		&hifi_volume_meter_fops);
	if (!entry_hifi_volume_meter)
		loge("Unable to create /proc/hifidsp/hifi_volume_meter entry\n");

	if (!entry_hifi && !entry_hifi_pcm_read && !entry_hifi_volume_meter ) {
...
static int hifi_misc_probe (struct platform_device *pdev)
{
	...
	spin_lock_init(&s_misc_data.para_write_lock);
	...
	s_misc_data.pcm_read_wait_flag = false;
	init_waitqueue_head(&s_misc_data.para_write_waitq);
	s_misc_data.para_write_wait_flag = false;
