...
static  void auxhear_bigdata_update_imonitor(unsigned int eventID, char *data, int len);
static  void hifi_om_auxhear_bigdata_handler(struct work_struct *work);
...
static  void hifi_om_work_smartpa_dft_handler(struct work_struct *work);
static  void hifi_om_work_audio_db_handler(struct work_struct *work);
...
static struct hifi_om_work_info work_info[] = {
	...
	{HIFI_OM_WORK_SMARTPA_DFT, "hifi_om_work_smartpa_dft_report",
		hifi_om_work_smartpa_dft_handler, {0}},
	{HIFI_OM_WORK_AUDIO_DB, "hifi_om_work_audio_db",
		hifi_om_work_audio_db_handler, {0}},
	{HIFI_OM_WORK_AUXHEAR_BIGDATA, "hifi_om_work_auxhear_bigdata",
		hifi_om_auxhear_bigdata_handler, {0}},
};
...
static int hifi_om_create_log_dir(char *path, int size)
{
...
static void hifi_om_show_voice_3a_info(struct work_struct *work)
{
	...
	case HIFI_3A_INFO_MSG:
		...
		#ifdef CONFIG_HUAWEI_DSM
		audio_dsm_report_info(AUDIO_CODEC, DSM_SOC_HIFI_3A_ERROR, "3a error type:%d error:%d nvid:%d dat1:%d dat2:%d\n",
			hifi_msg_type, voice_3a_om_info.recv_msg, voice_3a_om_info.nvid,
...
static void hifi_om_work_audio_db_handler(struct work_struct *work)
{
	...
	spin_lock_bh(&work_info[HIFI_OM_WORK_AUDIO_DB].ctl.lock);
	if (!list_empty(&work_info[HIFI_OM_WORK_AUDIO_DB].ctl.list)) {
		om_work = list_entry(
			work_info[HIFI_OM_WORK_AUDIO_DB].ctl.list.next,
			struct hifi_om_work, om_node);/*lint !e826*/
		memcpy(data, om_work->data, om_work->data_len);/*lint !e747*/ /* unsafe_function_ignore: memcpy */
		list_del(&om_work->om_node);
		kzfree(om_work);
	}
	spin_unlock_bh(&work_info[HIFI_OM_WORK_AUDIO_DB].ctl.lock);
...
static void hifi_om_auxhear_bigdata_handler(struct work_struct *work)
{
	...
	unsigned char data[MAIL_LEN_MAX] = {'\0'};
	unsigned int data_len = 0;
	static int auxhear_bigdata[3] = {0, 0, 0};

	static struct timespec bigdata_time1 = {0, 0};
	static struct timespec bigdata_time2 = {0, 0};

	spin_lock_bh(&work_info[HIFI_OM_WORK_AUXHEAR_BIGDATA].ctl.lock);
	if (!list_empty(&work_info[HIFI_OM_WORK_AUXHEAR_BIGDATA].ctl.list)) {
		om_work = list_entry(work_info[HIFI_OM_WORK_AUXHEAR_BIGDATA].ctl.list.next,
			struct hifi_om_work, om_node); /*lint !e826*/
		data_len = om_work->data_len;
		memcpy(data, om_work->data, om_work->data_len); /*lint !e747*/ /* unsafe_function_ignore: memcpy */
		list_del(&om_work->om_node);
	...
	spin_unlock_bh(&work_info[HIFI_OM_WORK_AUXHEAR_BIGDATA].ctl.lock);

	if ((sizeof(imedia_auxhear_bigdata)) != data_len) {
		logw("unavailable data from hifi, data_len: %u\n", data_len); //lint !e529
		return;
	}
	...
	if ((bigdata_time2.tv_sec - bigdata_time1.tv_sec) > AUXHEAR_IMONITOR_UPDATIME) {
		...
		memset(auxhear_bigdata, 0, sizeof(auxhear_bigdata)); /* unsafe_function_ignore: memset */
					...
					imedia_voice_bigdata_to_imonitor *voice_bigdata_buff, const char *blockmic)
{
	...
	static char *bigdata_voice_charact_param_hs[] = {
		"WhisperCnt", "AveCnt", "BweCnt", "AutolvmCnt", "WindCnt", "AngleCnt", "SvmCnt", "SvmoutCnt"
	};
...
static  void auxhear_bigdata_update_imonitor(unsigned int event_id, char *data, int len)
{
	...
	imedia_auxhear_bigdata *bigdata_mesg = (imedia_auxhear_bigdata*)data;

	if (bigdata_mesg == NULL) {
		logw(" imonitor data from kernel is empty \n"); //lint !e529
		return;
	}
	...
	if (auxhear_bigdata_obj == NULL) {
		logw(" imonitor obj create fail \n"); //lint !e529
		return;
	}

	/* carry  data from kerenl to imonitor */
	imonitor_set_param_integer_v2(auxhear_bigdata_obj, "AuxhearMode", bigdata_mesg->imedia_auxhear_mode);
	imonitor_set_param_integer_v2(auxhear_bigdata_obj, "AuxhearDevice", bigdata_mesg->imedia_auxhear_device);
	imonitor_set_param_integer_v2(auxhear_bigdata_obj, "AuxhearError", bigdata_mesg->imedia_auxhear_error);
...
static void hifi_dump_dsp(DUMP_DSP_INDEX index, char *log_path)
{
	...
	ret = hifi_om_create_log_dir(log_path, HIFI_DUMP_FILE_NAME_MAX_LEN);
	if (0 != ret) {
		goto END;
	}
...
static void hifi_init_workqueue(void)
{
	unsigned int i = 0;
