...
static ssize_t hwconn_read_proc_chiptype(struct file *filp,
...
static ssize_t hwconn_read_proc_nfc_chiptype(struct file *filp,
	char __user *buffer, size_t len, loff_t *off)
{
...
static int create_hwconn_proc_file(void)
{
	...
	hwconn_chiptype_file =
		proc_create(HW_CONN_PROC_CHIPTYPE_FILE, S_IRUGO,
		hwconn_dir, &hwconn_proc_fops_chiptype);
	...
	hwconn_chiptype_file =
		proc_create(HW_CONN_PROC_NFC_CHIPTYPE_FILE, S_IRUGO,
		hwconn_dir, &hwconn_proc_fops_nfc_chiptype);
	if (hwconn_chiptype_file == NULL) {
...
static int read_chip_type_from_dts(char *buf, int buf_len)
{
	...
	np = of_find_compatible_node(NULL, NULL, DTS_COMP_HW_CONNECTIVITY_NAME);
	if (np == NULL) {
...
static int read_nfc_chip_type_from_dts(char *buf, int buf_len)
{
	...
	/* should be the same as dts node compatible property */
	np = of_find_compatible_node(NULL, NULL, DTS_COMP_HISI_NFC_NAME);
	if (np == NULL) {
...
static int hw_connectivity_probe(struct platform_device *pdev)
{
	...
	ret = read_nfc_chip_type_from_dts(g_nfc_chip_type,
		sizeof(g_nfc_chip_type));
	if (ret < 0) {
...
static const struct of_device_id hw_connectivity_match_table[] = {
	{
		/* compatible must match with which defined in dts */
		.compatible = DTS_COMP_HW_CONNECTIVITY_NAME,
		.data = NULL,
	},
	{
	},
...
static struct platform_driver hw_connectivity_driver = {
	...
	.driver = {
		...
		.owner = THIS_MODULE,
		/* dts required code */
		.of_match_table = of_match_ptr(hw_connectivity_match_table),
	},
};
