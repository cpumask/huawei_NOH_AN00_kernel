...
static bool g_cps7181_mtp_latest;

static int cps7181_fw_unlock_i2c(void)
{
	return cps7181_aux_write_word(CPS7181_CMD_UNLOCK_I2C, CPS7181_I2C_CODE);
}

static int cps7181_fw_set_hi_addr(u32 hi_addr)
{
	return cps7181_aux_write_word(CPS7181_CMD_SET_HI_ADDR, hi_addr);
}

static int cps7181_fw_set_increase_mode(u16 mode)
{
	return cps7181_aux_write_word(CPS7181_CMD_INC_MODE, mode);
}

static int cps7181_fw_write_block(u32 addr, u8 *cmd, int len)
{
	int ret;
	u16 low_addr = addr & 0xFFFF; /* low 16bit addr */
	u16 hi_addr = addr >> 16; /* high 16bit addr */

	ret = cps7181_fw_set_hi_addr(hi_addr);
	if (ret)
		return ret;

	ret = cps7181_write_block(low_addr, cmd, len);
	if (ret)
		return ret;
...
static int cps7181_fw_read_block(u32 addr, u8 *data, int len)
{
	int ret;
	u16 low_addr = addr & 0xFFFF; /* low 16bit addr */
	u16 hi_addr = addr >> 16; /* high 16bit addr */
	...
	ret = cps7181_read_block(low_addr, data, len);
	if (ret)
		return ret;
...
static int cps7181_fw_hold_mcu(void)
{
	return cps7181_aux_write_word(CPS7181_CMD_HOLD_MCU, CPS7181_HOLD_MCU);
}
...
int cps7181_fw_sram_i2c_init(u8 inc_mode)
{
	...
	ret = cps7181_fw_unlock_i2c();
	if (ret)
		return ret;
	ret = cps7181_fw_set_increase_mode(inc_mode);
	if (ret)
		return ret;
	ret = cps7181_fw_set_hi_addr(CPS7181_SRAM_HI_ADDR);
	if (ret)
		return ret;

	ret = cps7181_write_block(CPS7181_I2C_TEST_ADDR,
		(u8 *)&wr_data, CPS7181_I2C_TEST_LEN);
	if (ret)
		return ret;
	ret = cps7181_read_block(CPS7181_I2C_TEST_ADDR,
...
static void cps7181_fw_soft_reset_system(void)
{
	u32 cmd = CPS7181_SYS_SOFT_REST;

	/* soft_reset: ignore i2c failure */
	(void)cps7181_fw_write_block(CPS7181_SYS_SOFT_REST_ADDR,
		(u8 *)&cmd, CPS7181_SYS_CMD_LEN);
	hwlog_err("reset_system: ignore i2c failure\n");
	msleep(100); /* sleep for system reset */
}

static int cps7181_fw_enable_remap(void)
{
	u32 cmd = CPS7181_SYS_REMAP_EN;

	return cps7181_fw_write_block(CPS7181_SYS_REMAP_EN_ADDR,
		(u8 *)&cmd, CPS7181_SYS_CMD_LEN);
}

static int cps7181_fw_disable_trim(void)
{
	u32 cmd = CPS7181_SYS_TRIM_DIS;

	return cps7181_fw_write_block(CPS7181_SYS_TRIM_DIS_ADDR,
		(u8 *)&cmd, CPS7181_SYS_CMD_LEN);
}

static int cps7181_fw_check_mtp_crc(void)
{
	int i;
	int ret;
	u16 crc = 0;
	...
	for (i = 0; i < 5; i++) {
		msleep(100);
		ret = cps7181_read_word(CPS7181_CRC_ADDR, &crc);
		if (ret) {
...
static int cps7181_fw_check_mtp_version(void)
{
	int ret;
	...
	ret = cps7181_get_chip_info(&info);
	if (ret) {
...
static int cps7181_fw_status_check(u32 cmd)
{
	int i;
	int ret;
	u8 status = 0;

	ret = cps7181_fw_write_block(CPS7181_SRAM_STRAT_CMD_ADDR,
	...
	for (i = 0; i < 10; i++) {
		msleep(50);
		ret = cps7181_fw_read_block(CPS7181_SRAM_CHK_CMD_ADDR,
			&status, BYTE_LEN);
		if (ret) {
...
static int cps7181_fw_write_sram_data(u32 cur_addr,
	const u8 *data, int len)
{
	...
	while (remaining > 0) {
		...
		ret = cps7181_write_block(addr, wr_buff, size_to_wr);
		if (ret) {
...
static int cps7181_fw_enter_load_mtp_mode(void)
{
	int ret;
	u32 test_en = CPS7181_PGM_EN_TEST;
	u32 carry_en = CPS7181_PGM_EN_CARRY;

	ret = cps7181_fw_write_block(CPS7181_PGM_EN_TEST_ADDR,
		(u8 *)&test_en, CPS7181_PGM_CMD_LEN);
	if (ret) {
		hwlog_err("enter_load_mtp_mode: enable test mode failed\n");
		return ret;
	}
	ret = cps7181_fw_write_block(CPS7181_PGM_EN_CARRY_ADDR,
...
static int cps7181_fw_exit_load_mtp_mode(void)
{
	int ret;
...
static int cps7181_fw_write_mtp_data(void)
{
	int ret;
	int offset = 0;
	...
	while (remaining > 0) {
		...
		ret = cps7181_fw_write_sram_data(CPS7181_SRAM_MTP_BUFF0,
			g_cps7181_mtp + offset, wr_size);
		if (ret) {
			hwlog_err("write_mtp_data: write mtp failed\n");
			return ret;
		}
		ret = cps7181_fw_status_check(CPS7181_STRAT_CARRY_BUF0);
		if (ret) {
...
static int cps7181_fw_load_mtp(void)
{
	int ret;

	ret = cps7181_fw_enter_load_mtp_mode();
	if (ret) {
	...
	ret = cps7181_fw_write_mtp_data();
	if (ret) {
		hwlog_err("load_mtp: write mtp data failed\n");
		return ret;
	}
	ret = cps7181_fw_exit_load_mtp_mode();
	if (ret) {
		hwlog_err("load_mtp: disable load_mtp failed\n");
		return ret;
	}
	ret = cps7181_fw_status_check(CPS7181_START_CHK_MTP);
	if (ret) {
...
static int cps7181_fw_check_bootloader_version(void)
{
	int ret;
	u16 btl_ver = 0;

	ret = cps7181_read_word(CPS7181_SRAM_BTL_VER_ADDR, &btl_ver);
	if (ret) {
...
static int cps7181_fw_check_bootloader(void)
{
	int ret;

	ret = cps7181_fw_sram_i2c_init(CPS7181_WORD_INC);
	if (ret) {
		hwlog_err("check_bootloader: i2c init failed\n");
		return ret;
	}
	ret = cps7181_fw_check_bootloader_version();
	if (ret) {
		hwlog_err("check_bootloader: check version failed\n");
		return ret;
	}
	ret = cps7181_fw_status_check(CPS7181_START_CHK_BTL);
	if (ret) {
...
static int cps7181_fw_load_bootloader(void)
{
	int ret;
...
static int cps7181_fw_progam_post_process(void)
{
	int ret;
	...
	ret = cps7181_fw_status_check(CPS7181_START_CHK_PGM);
	if (ret) {
		hwlog_err("progam_post_process: get program result failed\n");
		return ret;
	}

	cps7181_ps_control(WLPS_PROC_OTP_PWR, WLPS_CTRL_OFF);
	usleep_range(9500, 10500); /* delay 10ms for power off */
	cps7181_ps_control(WLPS_PROC_OTP_PWR, WLPS_CTRL_ON);
	msleep(100); /* delay 100ms for mcu restart */

	ret = cps7181_fw_check_mtp_version();
	if (ret) {
...
static int cps7181_fw_program_prev_process(void)
{
	int ret;
...
static int cps7181_fw_program_mtp(void)
{
	int ret;

	if (g_cps7181_mtp_latest)
		return 0;
	...
	ret = cps7181_fw_program_prev_process();
	if (ret)
		goto exit;
	ret = cps7181_fw_load_bootloader();
	if (ret)
		goto exit;
	ret = cps7181_fw_load_mtp();
	if (ret)
		goto exit;
	ret = cps7181_fw_progam_post_process();
	if (ret)
		goto exit;

	g_cps7181_mtp_latest = true;
...
static int cps7181_fw_rx_program_mtp(int proc_type)
{
	int ret;
	struct cps7181_dev_info *di = NULL;
...
static int cps7181_fw_check_mtp(void)
{
	if (g_cps7181_mtp_latest)
		return 0;
	...
	if (cps7181_fw_check_mtp_version()) {
		hwlog_err("check_mtp: mtp_ver mismatch\n");
		goto check_fail;
	}

	if (cps7181_fw_check_mtp_crc()) {
...
check_fail:
	cps7181_ps_control(WLPS_RX_EXT_PWR, WLPS_CTRL_OFF);
	return -WLC_ERR_CHECK_FAIL;
}
...
static int cps7181_fw_is_mtp_exist(void)
{
	int ret;
	struct cps7181_dev_info *di = NULL;
...
static struct wireless_fw_ops g_cps7181_fw_ops = {
	...
	.is_fw_exist            = cps7181_fw_is_mtp_exist,
	.check_fw               = cps7181_fw_check_mtp,
};

int cps7181_fw_ops_register(void)
{
	return wireless_fw_ops_register(&g_cps7181_fw_ops);
}
