...
#define IOSTAT_PID_HASH_LEN 1000 /* hash list length */
/* loop restriction for search directory entry in "/proc" */
#define IOSTAT_PID_SEARCH_CYC 100
...
#define IOSTAT_COMMANDLEN 16 /* pid name length */
...
struct io_node {
	/*
	 * the value indicate whether the node is using
	 * or the pid have killed
	 */
	int valid;
	int pid;   /* process pid */
...
/* proc container when "proc/ioinfo" requesta */
struct io_proc_info io_proc_inf = {0};
bool io_proc_request; /* proc request flag */
...
static struct io_node *io_top_get_ionode(int pid, char *comm)
{
	...
	hlist_for_each_entry(ion, phead, node) {
		if ((ion->pid == pid) &&
			(strncmp(ion->command, comm, IOSTAT_COMMANDLEN) == 0))
			break;
	}
 ...
 * @return: void
 */
static void io_top_set_ionode(struct io_node *old_ion,
					struct io_node *new_ion)
{
 ...
 * process have been killed, so scan all io node in the node list and delete
 * invalid node when io top stat have finished.
 * @param: void
 * @return: void
 */
 ...
 * @return: void
 */
static void io_top_save_proc_info(struct io_node *ion,
				u64 read_bytes, u64 write_bytes)
{
	...
	if (io_proc_inf.iotop_cnt < IOSTAT_TOP_LIMIT) {
		...
		memcpy(ion_proc->command, ion->command,
			sizeof(ion_proc->command));
 ...
 * @return: whether success or fail. 0:success, other is fail.
 */
static int io_top_get_tast_io(struct task_struct *task,
				struct io_node *ion, int whole)
{
 ...
 * @return: success or fail. 0:success, other is fail.
 */
static int io_top_get_taskinfo(struct io_node *ion, struct task_struct *task)
{
	...
	if ((ion == NULL) || (task == NULL))
		return ret;

	ion->pid = task->pid;

	get_task_comm(ion->command, task);
 ...
 * @param:proc_flag:save the pid's ioinfo to proc/ioinfo when this flag is true.
 * @return: void
...
static void io_top_stat(struct io_output_info *pio_inf, bool proc_flag)
{
	...
	list_for_each(pos, &initask->tasks) {
		...
		if (io_top_get_taskinfo(ion, task) != 0) {
			kfree(ion);
			continue;
		}

		/*
		 * Get old io node from hash list, then compare with
		 * new io node. 1.Add the new node to hash list when
		 * not have old node 2.Update the old node information
		 *  when the new node have increased r/w
		 */
		...
		if (old_ion == NULL) {
			...
			if (rchar || wchar || syscr || syscw ||
				read_bytes || write_bytes || cwrite_bytes)
		...
		if (BTOMB(read_bytes + write_bytes) > IOSTAT_TOP_THRESHOLD)
			io_top_save_io_info(pio_inf, ion,
						read_bytes, write_bytes);

		/*
		 * if have proc request, save io node information
		 * to proc container
		 */
 ...
 * @return: read bytes > 0: success; others: fail.
 */
static int io_dev_get_devstats(struct io_diskstats *pio_d)
{
	...
	if (ret > 0) {
		(void)sscanf(buf, "%lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu",
			&pio_d->read_tps,
			&pio_d->read_merged,
			&pio_d->read_sectors,
			&pio_d->read_time_ms,
...
static void io_dev_stat(struct io_output_info *pio_inf)
{
	...
	pio_inf->write_KB = (io_new_d.write_sectors -
			io_old_d.write_sectors) / 2;
...
static void io_wait_stat(struct io_output_info *pio_inf)
{
	...
	pio_inf->iowait = (u32)(inc.iowait * 100 /
				(inc.user + inc.nice + inc.system +
...
{
	io_pid_hash = (struct hlist_head *)kmalloc(
		sizeof(struct hlist_head) * IOSTAT_PID_HASH_LEN, GFP_KERNEL);
	if (io_pid_hash == NULL)
		return -ENOMEM;
...
static void io_data_stat_func(struct work_struct *work)
{
	...
	/*
	 * if have proc request, storage io information to io_info and
	 * proc container at the same time.
	 */
	if (io_proc_request)
		proc_flag = true;
	...
	queue_delayed_work(workqueue_stat_io, &stat_io_work,
				IOSTAT_TIMER_S * HZ);
}
...
{
	proc_create("ioinfo", 0000, NULL, &proc_info_operations);
