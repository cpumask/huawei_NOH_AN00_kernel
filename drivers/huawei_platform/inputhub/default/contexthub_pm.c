...
static int sensorhub_pm_resume(struct device *dev)
{
	...
	if (g_config_on_ddr->WrongWakeupMsg.flag) {
		g_config_on_ddr->WrongWakeupMsg.flag = 0;
		hwlog_err("************ sensorhub has wrong wakeup mesg\n");
		hwlog_err("time %llu\n", g_config_on_ddr->WrongWakeupMsg.time);
		hwlog_err("irqs [%x] [%x]\n",
			g_config_on_ddr->WrongWakeupMsg.irq0,
			g_config_on_ddr->WrongWakeupMsg.irq1);
		hwlog_err("recvfromapmsg [%x] [%x] [%x] [%x]\n",
			  g_config_on_ddr->WrongWakeupMsg.recvfromapmsg[0],
			  g_config_on_ddr->WrongWakeupMsg.recvfromapmsg[1],
			  g_config_on_ddr->WrongWakeupMsg.recvfromapmsg[2],
			  g_config_on_ddr->WrongWakeupMsg.recvfromapmsg[3]);
		hwlog_err("recvfromlpmsg [%x] [%x] [%x] [%x]\n",
			  g_config_on_ddr->WrongWakeupMsg.recvfromlpmsg[0],
			  g_config_on_ddr->WrongWakeupMsg.recvfromlpmsg[1],
			  g_config_on_ddr->WrongWakeupMsg.recvfromlpmsg[2],
			  g_config_on_ddr->WrongWakeupMsg.recvfromlpmsg[3]);
		hwlog_err("sendtoapmsg [%x] [%x] [%x] [%x]\n",
			  g_config_on_ddr->WrongWakeupMsg.sendtoapmsg[0],
			  g_config_on_ddr->WrongWakeupMsg.sendtoapmsg[1],
			  g_config_on_ddr->WrongWakeupMsg.sendtoapmsg[2],
			  g_config_on_ddr->WrongWakeupMsg.sendtoapmsg[3]);
		hwlog_err("sendtolpmsg [%x] [%x] [%x] [%x]\n",
			  g_config_on_ddr->WrongWakeupMsg.sendtolpmsg[0],
			  g_config_on_ddr->WrongWakeupMsg.sendtolpmsg[1],
			  g_config_on_ddr->WrongWakeupMsg.sendtolpmsg[2],
			  g_config_on_ddr->WrongWakeupMsg.sendtolpmsg[3]);
		hwlog_err("ap lpm3 tap tlpm3 %x %x %x %x\n",
			  g_config_on_ddr->WrongWakeupMsg.recvfromapmsgmode,
			  g_config_on_ddr->WrongWakeupMsg.recvfromlpmsgmode,
			  g_config_on_ddr->WrongWakeupMsg.sendtoapmsgmode,
			  g_config_on_ddr->WrongWakeupMsg.sendtolpmsgmode);
		hwlog_err("************ sensorhub has wrong wakeup mesg end\n");
		if (memset_s(&(g_config_on_ddr->WrongWakeupMsg), sizeof(wrong_wakeup_msg_t),
