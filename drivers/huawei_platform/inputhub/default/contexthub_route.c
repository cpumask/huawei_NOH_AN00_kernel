...
void als_calibrate_param_judge(struct als_device_info *dev_info, struct als_platform_data *pf_data)
{
	hwlog_info("als_calibrate_param_judge: phone = %d, x = %d,\n",
		pf_data->als_phone_type, dev_info->als_under_tp_cal_data.x);
	if ((pf_data->als_phone_type == NOAH) || (pf_data->als_phone_type == NOAHP)) {
		/* err param 71 */
		if (dev_info->als_under_tp_cal_data.x == 71) {
			/* default param 400,190 */
			dev_info->als_under_tp_cal_data.x = 400;
			dev_info->als_under_tp_cal_data.y = 190;
		}
	}
}
	...
	struct als_platform_data *pf_data, struct als_device_info *dev_info)
{
	...
	als_calibrate_param_judge(dev_info, pf_data);
