...
write_err:
	filp_close(fp, NULL);
#ifdef CONFIG_HISI_BB
	ret = (int)bbox_chown((const char __user *)path, ROOT_UID, SYSTEM_GID,
		false);
	if (ret)
...
static int write_sh_dump_file(void)
{
	...
	if (g_sensorhub_extend_dump_buff) {
		...
		sh_savebuf2fs(g_dump_dir, path, g_sensorhub_extend_dump_buff,
			min(pConfigOnDDr->dump_config.ext_dump_size, dzh->len),
			1);
	}
	return 0;
}
...
static int rdr_sh_thread(void *arg)
{
	...
	while (1) {
		...
		if (g_enable_dump) {
			...
			while (pConfigOnDDr->dump_config.finish !=
	...
	if (old_state == IOM3_RECOVERY_IDLE) {
		...
		if (g_enable_dump && g_dump_extend_size &&
			...
			if (g_sensorhub_extend_dump_buff) {
				...
				pConfigOnDDr->dump_config.ext_dump_size =
					g_dump_extend_size;
				barrier();
			}
		}
