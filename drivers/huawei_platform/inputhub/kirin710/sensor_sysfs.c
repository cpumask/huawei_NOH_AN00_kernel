...
static int enq_msg_data_in_sensorhub(struct sensor_eng_cal_test sensor_test)
{
	...
	while (pCalValue < sensor_test.value_num) {
		...
		for (pEvents = 0; pEvents < MAX_MSG_EVENT_NUM &&
			pCalValue < sensor_test.value_num; pEvents++, pCalValue++) {
			...
			if (pCalValue < sensor_test.threshold_num) {
				snprintf(msg->events[pEvents].min_threshold, MAX_VAL_LEN, "%d",
					*(sensor_test.min_threshold + pCalValue));
				snprintf(msg->events[pEvents].max_threshold, MAX_VAL_LEN, "%d",
					*(sensor_test.max_threshold + pCalValue));
			}
			memcpy(msg->events[pEvents].test_name, sensor_test.test_name[pCalValue],
...
{
	struct sensor_eng_cal_test cap_prox_test;
	...
	memset_s(&cap_prox_test, sizeof(cap_prox_test),
		0, sizeof(cap_prox_test));
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((txc_ps_flag != 1) && (ams_tmd2620_ps_flag != 1) &&
		...
		(ams_tmd3702_ps_flag != 1) && (ams_tof_flag != 1) &&
		(sharp_tof_flag != 1) && (ltr2568_ps_flag != 1) &&
...
static int ps_external_ir_power_set(unsigned long mode)
{
	int ret;
	...
	uint16_t value, uint16_t min_threshold, uint16_t max_threshold)
{
	...
	if (ret > 0)
		hwlog_info("%s succ!!item_id=%d\n", __func__, als_dark_noise_offset_event.item_id);
	else
		hwlog_info("%s failed!!\n", __func__);
#endif
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	int32_t minThreshold_als[ALS_CAL_NUM] = {
		minThreshold_als_para, minThreshold_als_para,
		minThreshold_als_para, minThreshold_als_para,
		minThreshold_als_para, minThreshold_als_para
	};
	int32_t maxThreshold_als[ALS_CAL_NUM] = {
		maxThreshold_als_para, maxThreshold_als_para,
		maxThreshold_als_para, maxThreshold_als_para,
		maxThreshold_als_para, maxThreshold_als_para
	};
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (fingersense_data_intrans && is_time_inrange(fingersense_data_ts,
		local_jiffies - FINGERSENSE_TRANS_TOUT, local_jiffies))
		skip = 1;

	/* The data was collected a short while ago. Just use it. */
	if (fingersense_data_ready && (is_time_inrange(fingersense_data_ts,
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < SENSOR_MAX; i++) {
		...
		if (detect_result == sensor_detect_flag)
			snprintf(&buf[i * MAX_SENSOR_NAME_LENGTH], MAX_SENSOR_NAME_LENGTH,
				"%s ", sensor_manager[i].sensor_name_str);
	}
