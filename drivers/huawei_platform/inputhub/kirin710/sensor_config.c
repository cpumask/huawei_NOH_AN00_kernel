...
void reset_calibrate_data(void)
{
	...
	if (rohm_rgb_flag == 1 || avago_rgb_flag == 1 ||
		...
		send_calibrate_data_to_mcu(TAG_ALS, SUB_CMD_SET_OFFSET_REQ,
			als_sensor_calibrate_data, ALS_CALIDATA_NV_SIZE, true);
...
void SetApds9308AlsExtendPrameters(void)
{
	...
	if ((g_apds9308Para[paraTable].apds9308_para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE)
		len = SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE;
	else
		len = g_apds9308Para[paraTable].apds9308_para;
	memcpy(als_data.als_extend_data,
		g_apds9308Para[paraTable].apds9308_para, len);
...
static void set_pinhole_als_extend_parameters(void)
{
	...
	hwlog_info("als_para_tab=%d phone_type=%d,sens_name=%d,phone_ver=%d\n",
		als_para_table, als_data.als_phone_type, sens_name,
		als_data.als_phone_version);
}
...
static void set_ltr578_als_extend_parameters(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(ltr578_als_table); i++) {
		if ((ltr578_als_table[i].phone_type ==
			als_data.als_phone_type) &&
			(ltr578_als_table[i].phone_version ==
			als_data.als_phone_version) &&
			(ltr578_als_table[i].sens_name == sens_name) &&
			((ltr578_als_table[i].tp_manufacture ==
			tp_manufacture) ||
			(ltr578_als_table[i].tp_manufacture ==
			TS_PANEL_UNKNOWN)) &&
			(ltr578_als_table[i].tp_color == phone_color)) {
			als_para_table = i;
			break;
		}
	}

	if (memcpy_s(als_data.als_extend_data,
		sizeof(als_data.als_extend_data),
		ltr578_als_table[als_para_table].para,
		(sizeof(ltr578_als_table[als_para_table].para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE) ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(ltr578_als_table[als_para_table].para)) != EOK) {
		hwlog_err("ltr578 memcpy als_extend_data error\n");
		return;
	}

	minThreshold_als_para =
		ltr578_als_table[als_para_table].para[LTR578_MIN_THD_NUM];
	maxThreshold_als_para =
		ltr578_als_table[als_para_table].para[LTR578_MAX_THD_NUM];
	hwlog_info("sens_name=%d, phone_type=%d, version=%d\n", sens_name,
...
static void set_stk3321_als_extend_parameters(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(stk3321_als_table); i++) {
		if ((stk3321_als_table[i].phone_type ==
			als_data.als_phone_type) &&
			(stk3321_als_table[i].phone_version ==
			als_data.als_phone_version) &&
			(stk3321_als_table[i].sens_name == sens_name) &&
			((stk3321_als_table[i].tp_manufacture ==
			tp_manufacture) ||
			(stk3321_als_table[i].tp_manufacture ==
			TS_PANEL_UNKNOWN)) &&
			(stk3321_als_table[i].tp_color == phone_color)) {
			als_para_table = i;
			break;
		}
	}
	...
	if (memcpy_s(als_data.als_extend_data,
		sizeof(als_data.als_extend_data),
		stk3321_als_table[als_para_table].para,
		(sizeof(stk3321_als_table[als_para_table].para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE) ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(stk3321_als_table[als_para_table].para)) != EOK) {
		hwlog_err("stk3321 memcpy als_extend_data error\n");
		return;
	}

	minThreshold_als_para =
		stk3321_als_table[als_para_table].para[STK3321_MIN_THD_NUM];
	maxThreshold_als_para =
		stk3321_als_table[als_para_table].para[STK3321_MAX_THD_NUM];
	hwlog_info("sens_name=%d, phone_type=%d, version=%d\n", sens_name,
		als_data.als_phone_type, als_data.als_phone_version);
	hwlog_info("table_index=%d, tp_color=%d, tp_manufacture=%d\n",
		als_para_table, phone_color, tp_manufacture);
...
static void set_stk3235_als_extend_parameters(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(stk3235_als_table); i++) {
		if ((stk3235_als_table[i].phone_type ==
			als_data.als_phone_type) &&
			(stk3235_als_table[i].phone_version ==
			als_data.als_phone_version) &&
			(stk3235_als_table[i].sens_name == sens_name) &&
			((stk3235_als_table[i].tp_manufacture ==
			tp_manufacture) ||
			(stk3235_als_table[i].tp_manufacture ==
			TS_PANEL_UNKNOWN)) &&
			(stk3235_als_table[i].tp_color == phone_color)) {
			als_para_table = i;
			break;
		}
	}
	...
	if (memcpy_s(als_data.als_extend_data,
		sizeof(als_data.als_extend_data),
		stk3235_als_table[als_para_table].para,
		(sizeof(stk3235_als_table[als_para_table].para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE) ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(stk3235_als_table[als_para_table].para)) != EOK) {
		hwlog_err("stk3235 memcpy als_extend_data error\n");
		return;
	}

	minThreshold_als_para =
		stk3235_als_table[als_para_table].para[STK3235_MIN_THD_NUM];
	maxThreshold_als_para =
		stk3235_als_table[als_para_table].para[STK3235_MAX_THD_NUM];
	hwlog_info("sens_name=%d, phone_type=%d, version=%d\n", sens_name,
		als_data.als_phone_type, als_data.als_phone_version);
	hwlog_info("table_index=%d, tp_color=%d, tp_manufacture=%d\n",
		als_para_table, phone_color, tp_manufacture);
...
static void set_sy3079_als_extend_parameters(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(sy3079_als_table); i++) {
		if ((sy3079_als_table[i].phone_type ==
			als_data.als_phone_type) &&
			(sy3079_als_table[i].phone_version ==
			als_data.als_phone_version) &&
			(sy3079_als_table[i].sens_name == sens_name) &&
			((sy3079_als_table[i].tp_manufacture ==
			tp_manufacture) ||
			(sy3079_als_table[i].tp_manufacture ==
			TS_PANEL_UNKNOWN)) &&
			(sy3079_als_table[i].tp_color == phone_color)) {
			als_para_table = i;
			break;
		}
	}
	...
	if (memcpy_s(als_data.als_extend_data,
		sizeof(als_data.als_extend_data),
		sy3079_als_table[als_para_table].para,
		(sizeof(sy3079_als_table[als_para_table].para) >
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE) ?
		SENSOR_PLATFORM_EXTEND_ALS_DATA_SIZE :
		sizeof(sy3079_als_table[als_para_table].para)) != EOK) {
		hwlog_err("sy3079 memcpy als_extend_data error\n");
		return;
	}

	minThreshold_als_para =
		sy3079_als_table[als_para_table].para[SY3079_MIN_THD_NUM];
	maxThreshold_als_para =
		sy3079_als_table[als_para_table].para[SY3079_MAX_THD_NUM];
	hwlog_info("sens_name=%d, phone_type=%d, version=%d\n", sens_name,
		als_data.als_phone_type, als_data.als_phone_version);
	hwlog_info("table_index=%d, tp_color=%d, tp_manufacture=%d\n",
		als_para_table, phone_color, tp_manufacture);
...
static void select_rohm_als_data(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(als_para_diff_tp_color_table); i++) {
		if ((als_para_diff_tp_color_table[i].phone_type ==
			als_data.als_phone_type) &&
			(als_para_diff_tp_color_table[i].phone_version ==
			als_data.als_phone_version) &&
			(als_para_diff_tp_color_table[i].tp_lcd_manufacture ==
			tplcd_manufacture ||
			als_para_diff_tp_color_table[i].tp_lcd_manufacture ==
			DEFAULT_TPLCD) &&
			(als_para_diff_tp_color_table[i].tp_color ==
			phone_color)) {
			als_para_table = i;
			break;
		}
...
static void select_avago_als_data(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(apds_als_para_diff_tp_color_table); i++) {
		if ((apds_als_para_diff_tp_color_table[i].phone_type ==
			als_data.als_phone_type) &&
			(apds_als_para_diff_tp_color_table[i].phone_version ==
			als_data.als_phone_version) &&
			(apds_als_para_diff_tp_color_table[i].tp_lcd_manufacture ==
			tplcd_manufacture ||
			apds_als_para_diff_tp_color_table[i].tp_lcd_manufacture ==
			DEFAULT_TPLCD) &&
			(apds_als_para_diff_tp_color_table[i].tp_color ==
			phone_color)) {
			als_para_table = i;
			break;
		}
...
static int read_tpcolor_from_nv(void)
{
	int ret;
	...
	if (g_sy3079_flag == 1)
		set_sy3079_als_extend_parameters();
	else if (g_stk3235_flag == 1)
		set_stk3235_als_extend_parameters();
	else if (g_stk3321_flag == 1)
		set_stk3321_als_extend_parameters();
	else if (ltr578_flag == 1)
		set_ltr578_als_extend_parameters();
...
int get_tpcolor_from_nv(void)
{
	...
	if (g_tp_color_from_nv_flag) {
		hwlog_info("%s: g_tp_color_from_nv_flag = %d\n",
			__func__, g_tp_color_from_nv_flag);
