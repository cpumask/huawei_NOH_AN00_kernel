...
static int unpack(const void *buf, unsigned int length)
{
	...
	((pkt_header_t *)buf)->partial_order = partial_order++;
...
int write_customize_cmd(const struct write_info *wr, struct read_info *rd, bool is_lock)
{
	...
	if ((wr->tag == TAG_SHAREMEM) && (g_iom3_state == IOM3_ST_REPEAT ||
		g_iom3_state == IOM3_ST_RECOVERY || iom3_power_state == ST_SLEEP))
...
static int report_resp_data(const pkt_subcmd_resp_t *head)
{
	...
	if (type_record.rd != NULL && type_record.pkt_info != NULL && /* check record info */
		...
		if (head->hd.length <= (MAX_PKT_LENGTH +
			...
			/* fill resp data to app */
			memcpy(type_record.rd->data, (char *)head + sizeof(pkt_subcmd_resp_t),
...
static int inputhub_process_motion_report(const pkt_header_t *head)
{
	...
	return inputhub_route_write(ROUTE_MOTION_PORT, motion_data,
		head->length - (sizeof(pkt_common_data_t) - sizeof(pkt_header_t)));
}
