...
static inline void dc_add_byte_packets(struct data_counters *counters, int set,
	enum ifs_tx_rx direction, enum ifs_proto ifs_proto,
...
static struct pid_node *pid_node_tree_search(struct rb_root *root, uint64_t tag)
{
...
static void pid_node_tree_insert(struct pid_node *data, struct rb_root *root)
{
	...
	while (*new) {
		struct pid_node *this = rb_entry(*new, struct pid_node, node);
		int result = tag_compare(data->tag, this->tag);

		rb_debug("qtup: %s(): tag=0x%llx (uid=%u)\n", __func__,
			this->tag,
...
static struct pid_stat *pid_stat_tree_search(struct rb_root *root, uint64_t tag)
{
...
{
	struct iface_pid_stat *new_iface = NULL;
	if (net_dev == NULL)
		return NULL;
...
void iface_pid_stat_create(struct net_device *net_dev, struct in_ifaddr *ifa)
{
	...
	struct iface_pid_stat *entry = NULL;
	__be32 ipaddr;
	struct iface_pid_stat *new_iface = NULL;

	if_debug("qtup: iface_pid_stat: create(%s): ifa=%p netdev=%p\n",
		net_dev ? net_dev->name : "?", ifa, net_dev);
	if (!net_dev) {
	...
	if (!ifa) {
		...
		if (!in_dev) {
			if_debug("qtup: create(%s): no indev\n", ifname);
			return;
		}
		if_debug("qtup: create(%s): in_dev=%p\n", ifname, in_dev);
		for (ifa = in_dev->ifa_list; ifa; ifa = ifa->ifa_next) {
			if_debug("qtup: iface_pid_stat: create(%s):ifa=%p "
	...
	if (!ifa) {
		if_debug("qtup: create(%s): no matching IP\n", ifname);
		goto done_put;
	}
	...
	if (entry != NULL) {
		if_debug("qtup: create(%s): entry=%p\n", ifname, entry);
		if_debug("qtup: %s(%s):tracking now %d on ip=%pI4\n", __func__,
			entry->ifname, true, &ipaddr);
		goto done_unlock_put;
	}

	new_iface = iface_pid_alloc(net_dev);
	if_debug("create(%s):entry=%p ip=%pI4\n", ifname, new_iface, &ipaddr);
done_unlock_put:
	...
	struct inet6_ifaddr *ifa)
{
	...
	entry = get_iface_pid_entry(ifname);
	if (entry != NULL) {
		if_debug("qtup: %s(%s):entry=%p\n", __func__, ifname, entry);
		if_debug("qtup: %s(%s):tracking now %d on ip=%pI6c\n", __func__,
			entry->ifname, true, &ifa->addr);
		goto done_unlock_put;
	}

	new_iface = iface_pid_alloc(net_dev);
	if_debug("qtup: iface_pid_stat: create6(%s): done entry=%p ip=%pI6c\n",
...
static struct pid_stat *create_if_pid_stat(struct iface_pid_stat *iface_entry,
	char *comm, uid_t real_uid, uint64_t tag)
{
...
static void pid_stat_update(struct pid_stat *pid_entry, int active_set,
	enum ifs_tx_rx direction, int proto, int bytes)
{
	mt_debug("qtup: pid_stat_update(pid=%s dir=%d proto=%d bytes=%d)\n",
		pid_entry->pn.comm, direction, proto, bytes);
	...
	enum ifs_tx_rx direction)
{
	...
	if (direction == IFS_TX) {
		skb_hash = jhash_3words(
			...
			(((uint32_t)hp->source) << UDP_HDR_MASK) |
			(uint32_t)hp->dest, 0);
	} else {
		skb_hash = jhash_3words(
			(uint32_t)iph->daddr, (uint32_t)iph->saddr,
			(((uint32_t)hp->dest) << UDP_HDR_MASK) |
			(uint32_t)hp->source, 0);
	}
...
void if_pid_stat_update(const char *ifname, uid_t uid,
	...
	int proto, int bytes)
{
	struct pid_stat *pid_stat_entry = NULL;
	char task_comm[TASK_COMM_LEN] = {0};
	uint32_t task_comm_hash;
	uid_t real_uid;
	uint64_t tag;
	uint64_t pid_tag;
	struct iface_pid_stat *iface_entry = NULL;
	...
	if (!iface_entry) {
		mt_debug("qtup:stat_update() %s not found\n", ifname);
		spin_unlock_bh(&iface_pid_stat_list_lock);
		return;
	}
...
static int pp_stats_line(struct seq_file *m, struct pid_stat *ps_entry, int cnt_set)
{
	struct proc_print_info *ppi = NULL;
	...
	ct_debug("line:%s:pid=%s\n", ppi->iface_entry->ifname, ps_entry->pn.comm);
	...
	seq_printf(m, "%d %s %s %u 0x%llx %u %u "
		...
		"%llu %llu %llu %llu\n", ppi->item_index,
		ppi->iface_entry->ifname, comm, real_uid, make_atag_from_value(0),
		tag_uid, cnt_set, dc_sum_bytes(cnts, cnt_set, IFS_RX),
		dc_sum_packets(cnts, cnt_set, IFS_RX), dc_sum_bytes(cnts,
		cnt_set, IFS_TX), dc_sum_packets(cnts, cnt_set, IFS_TX),
		cnts->bpc[cnt_set][IFS_RX][IFS_TCP].bytes, cnts->bpc[cnt_set][IFS_RX][IFS_TCP].packets,
		cnts->bpc[cnt_set][IFS_RX][IFS_UDP].bytes, cnts->bpc[cnt_set][IFS_RX][IFS_UDP].packets,
...
static void qtaguid_pid_stats_proc_next_iface_entry(
