	...
	const char *url, char *ip)
{
	...
	if (temp == NULL)
		return NULL;
	snprintf(temp, len + ULR_LEN_OFFSET_64 + 1, "%s", url);
	len = strlen(temp);
...
void nofity_loop(unsigned int uid, unsigned int dlid, const char *url, int action)
{
	struct dl_info *node = NULL;
	int len = strlen(url);
	...
	list_for_each_entry(node, &g_dl_wait, list) {
		if (uid == node->uid && len == node->len &&
			...
			pr_info("hwad:nofity_loop uid=%d dlid=%d act=%d\n",
				node->uid, node->dlid, node->action);
		}
	}
}
