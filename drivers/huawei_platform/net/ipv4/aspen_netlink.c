...
static void nl_do_recv_msg(struct sk_buff *skb)
{
	...
	aspen_info("Aspen netlink received msg from userland.(pid=%d)",
		aspen_nl_entry.userland_pid);
}

/* Note: This is called from process context only because of mutex_lock */
static void aspen_nl_recv_from_userland(struct sk_buff *skb)
{
	if (!skb) {
		aspen_err("Invalid parameter: zero pointer reference.(skb)");
		return;
...
static int aspen_nl_send_to_userland(struct tcp_sender_monitor *msg)
{
	struct nlmsghdr *nlh = NULL;
	...
	if (res < 0) {
		...
		aspen_err("An error %d occurred while sending msg to userland",
			res);
	}
