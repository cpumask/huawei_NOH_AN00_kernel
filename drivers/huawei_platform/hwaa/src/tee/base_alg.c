	...
	const u8 *in, u32 in_len, u8 *out, u32 *out_len, bool enc)
{
	...
	if (!check_aes_cbc_para(key, key_len, iv, iv_len, in, in_len, out,
		...
		return -EINVAL;
#ifdef CONFIG_VMAP_STACK
	in_dynamic = kzalloc(in_len, GFP_NOFS);
	out_dynamic = kzalloc(in_len, GFP_NOFS); // out len is no longer than in len
	if (!in_dynamic || !out_dynamic) {
		res = -ENOMEM;
		goto free_0;
	}
	if(memcpy_s(in_dynamic, in_len, in, in_len) != EOK) {
		res = -EINVAL;
		goto free_0;
	}
#endif
	...
	if (res < 0)
		goto free_2;
#ifdef CONFIG_VMAP_STACK
	sg_init_one(&src_sg, in_dynamic, in_len);
	sg_init_one(&dst_sg, out_dynamic, in_len);
#else
	...
	if (!res) {
#ifdef CONFIG_VMAP_STACK
		if (memcpy_s(out, *out_len, out_dynamic, in_len) == EOK)
			*out_len = in_len;
		else
			res = -EINVAL;
#else
