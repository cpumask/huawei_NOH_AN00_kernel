	...
	struct usb_host_endpoint **out_ep, struct usb_host_endpoint **in_ep)
{
	...
	if (ss->intf->cur_altsetting != altsetting) {
		D("usb_set_interface: interface %u, altsetting %u\n",
	...
	bool is_in)
{
	...
	D("pkt_num %u, pkt_len %u, urb_num %u\n", test->pkt_num,
...
static void __auto_isoc_test(struct sourcesink *ss, bool is_in)
{
	...
	for (i = 0; isoc_tcs[i].pkt_len != 0; i++) {
		...
		/* us to ms, /1000 */
		D("interval %lu ms, mps %u\n", interval / 1000, mps);
		delay_ms = (interval * isoc_tc->pkt_num *
			isoc_tc->urb_num) / 1000;
		D("Wait %u ms for transfer complete.\n", delay_ms);
		msleep(delay_ms + 1000); /* wait more 1000ms */
	}
}
