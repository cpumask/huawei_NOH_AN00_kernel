...
#define LOG_TAG "[PD][pd_process_evt_prs.c]"
...
DECL_PE_STATE_TRANSITION(PD_HW_VBUS_SAFE0V) = {				/* comming event */
	{ PE_PRS_SRC_SNK_TRANSITION_TO_OFF, PE_PRS_SRC_SNK_ASSERT_RD },	/* current state, next state */
};
...
static inline bool pd_process_timer_msg(pd_port_t *pd_port, pd_event_t *pd_event)
{
	...
	case PD_TIMER_PS_SOURCE_OFF: /* this timer wair for PS_RDY from source */
		return PE_MAKE_STATE_TRANSIT(PD_TIMER_PS_SOURCE_OFF);

	case PD_TIMER_SOURCE_TRANSITION:
		hisi_pd_dpm_prs_enable_power_source(pd_port, false); /* stop source vbus and wait for vsafe0V */
		D("-\n");
		return false;
	default:
		D("-\n");
...
bool hisi_hisi_pd_process_event_prs(pd_port_t *pd_port, pd_event_t *pd_event)
{
	bool ret = false;
