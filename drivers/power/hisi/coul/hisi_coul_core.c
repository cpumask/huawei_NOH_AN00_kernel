		...
		int batt_temp, int ocv)
{
	...
	pcj = 0;
	for (i = 0; i < rows - 1; i++) {
		if (pcj == 0 && is_between(lut->ocv[i][j],
				lut->ocv[i + 1][j], ocv))
			pcj = linear_interpolate(
				      lut->percent[i] * PERMILLAGE,
				      lut->ocv[i][j],
		...
		if (pcj_minus_one == 0 && is_between(lut->ocv[i][j - 1],
				lut->ocv[i + 1][j - 1], ocv))
...
coul_no_battery:
	...
	coul_ops->get_battery_temperature_raw = coul_get_battery_temperature_raw;
	coul_ops->get_battery_temp_permille = coul_battery_temperature_permille;
	coul_ops->get_c_calibration = get_cur_calibration;
	di->ops = coul_ops;
