...
kernel_out_dir=${kernel_sys_dir}${kernel_system_tree##${topdir}}
...
echo "generate link-vmlinux-new.sh"
...
sed -i "/TMPDIR=/d" ${kernel_out_dir}/link-vmlinux-new.sh
...
echo "cp -pf ${objtree}/arch/${SRCARCH}/boot/${KBUILD_IMAGE} ${OBB_PRODUCT_DELIVERY_DIR}/image/kernel" >> ${kernel_out_dir}/link-vmlinux-new.sh
