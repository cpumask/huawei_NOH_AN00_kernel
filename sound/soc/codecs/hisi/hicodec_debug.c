...
static struct snd_soc_codec *g_codec[ASP_CODEC_CNT];
...
static bool is_valid_codec(struct snd_soc_codec **codec)
{
	int i;
...
static void dump_dapm_widget(char *buf, struct snd_soc_codec *codec, size_t buf_len)
{
	...
	dc = snd_soc_codec_get_dapm(codec);
#else
	dc = &(codec->dapm);
#endif
	if (!dc || !(dc->card)) {
...
static void dump_dapm_path(char *buf, struct snd_soc_codec *codec, size_t buf_len)
{
	struct list_head *l = NULL;
...
static void dump_kcontrol(char *buf, struct snd_soc_codec *codec, size_t buf_len)
{
	struct list_head *l = NULL;
...
static void dump_rr_page(char *buf, size_t length, struct snd_soc_codec *codec, const struct hicodec_dump_reg_info *info)
{
	unsigned int regi;
	unsigned int i;
	size_t content_len;
	...
	for (i = 0; i < info->count; ++i) {
		...
		for (regi = entry->start; regi <= entry->end; regi += entry->reg_size) {
			...
			snprintf(buf + content_len, length - content_len,
				"w 0x%04X 0x%08X\n", regi, codec->driver->read(codec, regi));
		}
	}
}
		...
		size_t count, loff_t *ppos)
{
	...
	if ((!g_codec[ASP_CODECLESS] || !(g_codec[ASP_CODECLESS]->driver) || !(g_codec[ASP_CODECLESS]->driver->read)) &&
		(!g_codec[ASP_CODEC] || !(g_codec[ASP_CODEC]->driver) || !(g_codec[ASP_CODEC]->driver->read))) {
		pr_err(LOG_TAG"g_codec or g_codec->driver or g_codec->driver->read is null\n");
		return -EINVAL;
	}

	length = HICODEC_DBG_SIZE_PAGE;
	codec = (struct snd_soc_codec *)g_codec[ASP_CODEC];
	...
	if((g_vs_reg >= reg_info[ASP_CODECLESS]->entry->start) && (g_vs_reg <= reg_info[ASP_CODECLESS]->entry->end))
		codec = (struct snd_soc_codec *)g_codec[ASP_CODECLESS];
#endif
	...
	if (isReadRegAll) {
		...
		snprintf(kn_buf + strlen(kn_buf), HICODEC_DBG_SIZE_PAGE - strlen(kn_buf),
				"%#04x:%#010x\n", g_vs_reg, codec->driver->read(codec, g_vs_reg));
	}
...
static ssize_t hicodec_debug_rr_write(struct file *file, const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if ((!g_codec[ASP_CODECLESS] || !(g_codec[ASP_CODECLESS]->driver) || !(g_codec[ASP_CODECLESS]->driver->write)) &&
		(!g_codec[ASP_CODEC] || !(g_codec[ASP_CODEC]->driver) || !(g_codec[ASP_CODEC]->driver->write))) {
...
#endif
		if (2 == num) {
			codec->driver->write(codec, vs_reg, vs_val);
