...
#define HISI_PCM_CP_MAX_CHANNELS 13
/* Assume the FIFO size */
...
static struct snd_soc_dai_driver hisi_pcm_dai[] =
{
	...
	},
	{
		...
		.capture = {
			.stream_name  = "hi6210-modem Capture",
			.channels_min = HISI_PCM_CP_MIN_CHANNELS,
			.channels_max = HISI_PCM_CP_MAX_CHANNELS,
			.rates        = HISI_PCM_MODEM_RATES,
			.formats      = HISI_PCM_CP_FORMATS
...
static void hisi_pcm_isr_handle(struct snd_pcm_substream *substream,
	...
	for (device = 0; device < PCM_DEVICE_TOTAL; device++) {
		for (stream = 0; stream < PCM_STREAM_MAX; stream++) {
			...
			if (_is_one_frame_finished(thread_info, prtd->substream)) {
				hisi_pcm_isr_handle(prtd->substream, 1, false);
...
static void update_pcm_buff(struct snd_pcm_substream *substream, bool discard)
{
	...
	if (avail < rt_period_size) {
		if (hifi_misc_get_platform_type() == HIFI_DSP_PLATFORM_ASIC)
			logw("underrun, device is %d, mode is %d, avail[%ld] < period size[%ld]\n",
	...
	if (!discard) {
		...
		if (ret < 0)
			loge("mode is %d, device is %d: notify set buf fail, ret is %d\n",
...
static void print_pcm_timeout(unsigned int pre_time, const char *print_type, unsigned int time_delay)
{
	...
	if (delay_time > (HISI_PCM_WORK_DELAY_1MS * time_delay)) {
		logw("%s, delay time stamp diff is %u\n", print_type, delay_time);
	}
}
	...
	struct mb_queue *mail_handle, unsigned int mail_len)
{
	...
	switch(mail_buf.msg_type) {
		case HI_CHN_MSG_PCM_PERIOD_ELAPSED:
			...
			hisi_pcm_isr_handle(substream, mail_buf.frame_num, (bool)mail_buf.discard);
		...
		struct snd_pcm_hw_params *params)
{
	...
	logi("device %u, mode %u, low power support %u,"
...
static int  hisi_pcm_platform_probe(struct platform_device *pdev)
{
	...
	if (dma_buffer_end_addr > PCM_OM_BUFF_LOCATION) {
		loge("data memory across the border\n");
		goto probe_failed;
	}
